...
--- 素材
    local assets =
    {
        ...
        Asset("IMAGE", "images/hoshino_mission/y_mission_b_05.tex"),
        Asset("ATLAS", "images/hoshino_mission/y_mission_b_05.xml"),
        Asset("IMAGE", "images/hoshino_mission/y_mission_s_05.tex"),
        Asset("ATLAS", "images/hoshino_mission/y_mission_s_05.xml"),
    }
...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_golden_05","hoshino_mission_golden_05")
        inst:ListenForEvent("hoshino_mission_golden_05",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/y_mission_s_05.xml","y_mission_s_05.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_golden_05",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/y_mission_b_05.xml","y_mission_b_05.tex"))
        ------- 任务描述
    ...
    local function Task_Delivery_Event_Install(inst)
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            inst:ListenForEvent("hoshino_event.exp_broadcast",function(_,_table)
                if not (_table and _table.prefab == "crabking") then
                    return
                end
...
return Prefab("hoshino_mission_golden_05", fn, assets)