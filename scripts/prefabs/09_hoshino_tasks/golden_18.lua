...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_golden_18","hoshino_mission_golden_18")
        inst:ListenForEvent("hoshino_mission_golden_18",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_18_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            local update_fn = function()
                ...
                display_text:SetString(""..num.."/28")
            end
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_golden_18",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_18_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 28 then
                inst:Remove()
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            -- 击杀监听
            inst.__targets = {}
            inst:ListenForEvent("hoshino_mission_golden_18_kill_with_debuff",function(_,target)
                if inst.__targets[target] == nil then
                    inst.__targets[target] = true
                    local num = inst.components.hoshino_data:Add("num",1,0,28)
                    inst.__num:set(num)
                    if num >= 28 then
                        owner:PushEvent("hoshino_event.pad_warnning","main_page")
                    end
            ...
            local function add_debuff_2_monster(monster)
                local debuff_prefab = "hoshino_mission_golden_18_debuff"
                local test_num = 100
                while test_num > 0 do
            ...
            local function GetSpiders()
                ...
                local list = {
                    ...
                    ["spider_healer"] = 8,      -- 护士蜘蛛
                }
                local num = 0
                for i = 1, 20, 1 do
                    if list["spider_spitter"] > 0 then
                        table.insert(ret_table,"spider_spitter")
                        list["spider_spitter"] = list["spider_spitter"] - 1
                        num = num + 1
                    end
                    if list["spider_warrior"] > 0 then
                        table.insert(ret_table,"spider_warrior")
                        list["spider_warrior"] = list["spider_warrior"] - 1
                        num = num + 1
                    end
                    if list["spider_healer"] > 0 then
                        table.insert(ret_table,"spider_healer")
                        list["spider_healer"] = list["spider_healer"] - 1
                        num = num + 1
                    end
                end
        ...
        --- 死亡广播
            target:ListenForEvent("minhealth",function()
                ...
                for k, v in pairs(ents) do
                    v:PushEvent("hoshino_mission_golden_18_kill_with_debuff",target)
                end
            end)
            ...
            --         local ents = TheSim:FindEntities(x,0, z, 100, {"player"})
            --         for k, v in pairs(ents) do
            --             v:PushEvent("hoshino_mission_golden_15_kill_with_debuff",target)
            --         end
            --     end
            -- end,TheWorld)
            target.hoshino_mission_golden_15_debuff = true
        -----------------------------------------------------
        ---
...
return Prefab("hoshino_mission_golden_18", fn, assets),
    Prefab("hoshino_mission_golden_18_debuff", debuff_fn, assets)