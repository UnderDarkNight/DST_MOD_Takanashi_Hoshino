...
--- 素材
    local assets =
    {
        ...
        Asset("IMAGE", "images/hoshino_mission/y_mission_b_24.tex"),
        Asset("ATLAS", "images/hoshino_mission/y_mission_b_24.xml"),
        Asset("IMAGE", "images/hoshino_mission/y_mission_s_24.tex"),
        Asset("ATLAS", "images/hoshino_mission/y_mission_s_24.xml"),
    }
...
--- 专属参数
    ...
    local MISSION_ITEM_1 = "redmooneye"
    local MISSION_ITEM_1_NUM = 1
    local MISSION_ITEM_2 = "greenmooneye"
    local MISSION_ITEM_2_NUM = 1
    local MISSION_ITEM_3 = "bluemooneye"
...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__redmooneye_num = net_uint(inst.GUID, "hoshino_mission_golden_24.redmooneye","hoshino_mission_golden_24")
        inst.__greenmooneye_num = net_uint(inst.GUID, "hoshino_mission_golden_24.greenmooneye","hoshino_mission_golden_24")
        inst.__bluemooneye_num = net_uint(inst.GUID, "hoshino_mission_golden_24.bluemooneye","hoshino_mission_golden_24")
        inst:ListenForEvent("hoshino_mission_golden_24",function()
            inst.redmooneye_num = inst.__redmooneye_num:value()
            inst.greenmooneye_num = inst.__greenmooneye_num:value()
            inst.bluemooneye_num = inst.__bluemooneye_num:value()
        end)
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/y_mission_s_24.xml","y_mission_s_24.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local delta_y = -22
            local front_size = 25
            local redmooneye_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            redmooneye_text:SetPosition(x,y)

            local greenmooneye_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            greenmooneye_text:SetPosition(x,y+delta_y)

            local bluemooneye_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            bluemooneye_text:SetPosition(x,y+delta_y*2)
        --------------------------------------------------------------------------
        --- 检查任务是否完成
            local update_fn = function()
                local redmooneye_flag,redmooneye_num = Has_Enough_Items(ThePlayer,"redmooneye",MISSION_ITEM_1_NUM)
                local greenmooneye_flag,greenmooneye_num = Has_Enough_Items(ThePlayer,"greenmooneye",MISSION_ITEM_2_NUM)
                local bluemooneye_flag,bluemooneye_num = Has_Enough_Items(ThePlayer,"bluemooneye",MISSION_ITEM_3_NUM)
                if redmooneye_flag and greenmooneye_flag and bluemooneye_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                redmooneye_num = math.clamp(redmooneye_num,0,MISSION_ITEM_1_NUM)
                greenmooneye_num = math.clamp(greenmooneye_num,0,MISSION_ITEM_2_NUM)
                bluemooneye_num = math.clamp(bluemooneye_num,0,MISSION_ITEM_3_NUM)
                redmooneye_text:SetString(""..redmooneye_num.."/"..MISSION_ITEM_1_NUM)
                greenmooneye_text:SetString(""..greenmooneye_num.."/"..MISSION_ITEM_2_NUM)
                bluemooneye_text:SetString(""..bluemooneye_num.."/"..MISSION_ITEM_3_NUM)
            end
            update_fn()
            redmooneye_text.inst:ListenForEvent("hoshino_mission_golden_24",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/y_mission_b_24.xml","y_mission_b_24.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"redmooneye",MISSION_ITEM_1_NUM) and Has_Enough_Items(owner,"greenmooneye",MISSION_ITEM_2_NUM) and Has_Enough_Items(owner,"bluemooneye",MISSION_ITEM_3_NUM) then
                inst:Remove()
                ...
                Remove_Items_By_Prefab(owner,"bluemooneye",MISSION_ITEM_3_NUM)
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local redmooneye_flag,redmooneye_num = Has_Enough_Items(owner,"redmooneye",MISSION_ITEM_1_NUM)
                local greenmooneye_flag,greenmooneye_num = Has_Enough_Items(owner,"greenmooneye",MISSION_ITEM_2_NUM)
                local bluemooneye_flag,bluemooneye_num = Has_Enough_Items(owner,"bluemooneye",MISSION_ITEM_3_NUM)

                redmooneye_num = math.clamp(redmooneye_num,0,MISSION_ITEM_1_NUM)
                greenmooneye_num = math.clamp(greenmooneye_num,0,MISSION_ITEM_2_NUM)
                bluemooneye_num = math.clamp(bluemooneye_num,0,MISSION_ITEM_3_NUM)

                inst.__redmooneye_num:set(redmooneye_num)
                inst.__greenmooneye_num:set(greenmooneye_num)
                inst.__bluemooneye_num:set(bluemooneye_num)

                if redmooneye_num >= MISSION_ITEM_1_NUM and greenmooneye_num >= MISSION_ITEM_2_NUM and bluemooneye_num >= MISSION_ITEM_3_NUM then
...
return Prefab("hoshino_mission_golden_24", fn, assets)