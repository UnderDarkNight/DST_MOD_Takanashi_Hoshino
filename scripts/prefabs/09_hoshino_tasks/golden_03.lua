...
--- 专属参数
    ...
    local MISSION_ITEM_1 = "pigskin"
    local MISSION_ITEM_1_NUM = 1
    local MISSION_ITEM_2 = "slurper_pelt"
    local MISSION_ITEM_2_NUM = 1
    local MISSION_ITEM_3 = "bearger_fur"
...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__pigskin_num = net_uint(inst.GUID, "hoshino_mission_golden_03.pigskin","hoshino_mission_golden_03")
        inst.__slurper_pelt_num = net_uint(inst.GUID, "hoshino_mission_golden_03.slurper_pelt","hoshino_mission_golden_03")
        inst.__bearger_fur_num = net_uint(inst.GUID, "hoshino_mission_golden_03.bearger_fur","hoshino_mission_golden_03")
        inst:ListenForEvent("hoshino_mission_golden_03",function()
            inst.pigskin_num = inst.__pigskin_num:value()
            inst.slurper_pelt_num = inst.__slurper_pelt_num:value()
            inst.bearger_fur_num = inst.__bearger_fur_num:value()
        end)
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_03_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local delta_y = -22
            local front_size = 25
            local pigskin_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            pigskin_text:SetPosition(x,y)

            local slurper_pelt_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            slurper_pelt_text:SetPosition(x,y+delta_y)

            local bearger_fur_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            bearger_fur_text:SetPosition(x,y+delta_y*2)
        --------------------------------------------------------------------------
        --- 检查任务是否完成
            local update_fn = function()
                local pigskin_flag,pigskin_num = Has_Enough_Items(ThePlayer,"pigskin",MISSION_ITEM_1_NUM)
                local slurper_pelt_flag,slurper_pelt_num = Has_Enough_Items(ThePlayer,"slurper_pelt",MISSION_ITEM_2_NUM)
                local bearger_fur_flag,bearger_fur_num = Has_Enough_Items(ThePlayer,"bearger_fur",MISSION_ITEM_3_NUM)
                if pigskin_flag and slurper_pelt_flag and bearger_fur_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                pigskin_num = math.clamp(pigskin_num,0,MISSION_ITEM_1_NUM)
                slurper_pelt_num = math.clamp(slurper_pelt_num,0,MISSION_ITEM_2_NUM)
                bearger_fur_num = math.clamp(bearger_fur_num,0,MISSION_ITEM_3_NUM)
                pigskin_text:SetString(""..pigskin_num.."/"..MISSION_ITEM_1_NUM)
                slurper_pelt_text:SetString(""..slurper_pelt_num.."/"..MISSION_ITEM_2_NUM)
                bearger_fur_text:SetString(""..bearger_fur_num.."/"..MISSION_ITEM_3_NUM)
            end
            update_fn()
            pigskin_text.inst:ListenForEvent("hoshino_mission_golden_03",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_03_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"pigskin",MISSION_ITEM_1_NUM) and Has_Enough_Items(owner,"slurper_pelt",MISSION_ITEM_2_NUM) and Has_Enough_Items(owner,"bearger_fur",MISSION_ITEM_3_NUM) then
                inst:Remove()
                ...
                Remove_Items_By_Prefab(owner,"bearger_fur",MISSION_ITEM_3_NUM)
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local pigskin_flag,pigskin_num = Has_Enough_Items(owner,"pigskin",MISSION_ITEM_1_NUM)
                local slurper_pelt_flag,slurper_pelt_num = Has_Enough_Items(owner,"slurper_pelt",MISSION_ITEM_2_NUM)
                local bearger_fur_flag,bearger_fur_num = Has_Enough_Items(owner,"bearger_fur",MISSION_ITEM_3_NUM)

                pigskin_num = math.clamp(pigskin_num,0,MISSION_ITEM_1_NUM)
                slurper_pelt_num = math.clamp(slurper_pelt_num,0,MISSION_ITEM_2_NUM)
                bearger_fur_num = math.clamp(bearger_fur_num,0,MISSION_ITEM_3_NUM)

                inst.__pigskin_num:set(pigskin_num)
                inst.__slurper_pelt_num:set(slurper_pelt_num)
                inst.__bearger_fur_num:set(bearger_fur_num)

                if pigskin_num >= MISSION_ITEM_1_NUM and slurper_pelt_num >= MISSION_ITEM_2_NUM and bearger_fur_num >= MISSION_ITEM_3_NUM then
...
return Prefab("hoshino_mission_golden_03", fn, assets)