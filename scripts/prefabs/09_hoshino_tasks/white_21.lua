...
--- 素材
    local assets =
    {
        ...
        Asset("IMAGE", "images/hoshino_mission/w_mission_b_21.tex"),
        Asset("ATLAS", "images/hoshino_mission/w_mission_b_21.xml"),
        Asset("IMAGE", "images/hoshino_mission/w_mission_s_21.tex"),
        Asset("ATLAS", "images/hoshino_mission/w_mission_s_21.xml"),
    }
...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_white_21","hoshino_mission_white_21")
        inst:ListenForEvent("hoshino_mission_white_21",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/w_mission_s_21.xml","w_mission_s_21.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---  91,112,136
            ...
            display_text:SetPosition(-300+10,-30) -- 两位数 需要偏移 10
        --------------------------------------------------------------------------
        --- 检查任务是否完成
            local update_fn = function()
                ...
                display_text:SetString(""..num.."/50")
            end
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_white_21",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/w_mission_b_21.xml","w_mission_b_21.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 50 then
                inst:Remove()
                ...
                local exp = current_max_exp*0.5 -- 50% 经验
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            inst:DoPeriodicTask(5,function()
                if inst.components.hoshino_data:Add("num",0) >= 50 then
...
return Prefab("hoshino_mission_white_21", fn, assets)