...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__birdtrap_num = net_uint(inst.GUID, "hoshino_mission_white_26.birdtrap","hoshino_mission_white_26")
        inst.__trap_num = net_uint(inst.GUID, "hoshino_mission_white_26.trap","hoshino_mission_white_26")
        inst:ListenForEvent("hoshino_mission_white_26",function()
            inst.birdtrap_num = inst.__birdtrap_num:value()
            inst.trap_num = inst.__trap_num:value()
        end)
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/white_mission.xml","white_mission_26_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local trap_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            trap_text:SetPosition(x,y)

            local birdtrap_text = bg:AddChild(Text(CODEFONT,front_size,"0/3",{ 91/255 , 112/255 ,136/255 , 1}))
            birdtrap_text:SetPosition(x,y+delta_y)

        --------------------------------------------------------------------------
        --- 检查任务是否完成
            local update_fn = function()
                local birdtrap_flag,birdtrap_num = Has_Enough_Items(ThePlayer,"birdtrap",3)
                local trap_flag,trap_num = Has_Enough_Items(ThePlayer,"trap",1)

                if birdtrap_flag and trap_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                birdtrap_num = math.clamp(birdtrap_num,0,3)
                trap_num = math.clamp(trap_num,0,1)

                birdtrap_text:SetString(""..birdtrap_num.."/3")
                trap_text:SetString(""..trap_num.."/1")
            end
            update_fn()
            birdtrap_text.inst:ListenForEvent("hoshino_mission_white_26",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/white_mission.xml","white_mission_26_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"birdtrap",3) and Has_Enough_Items(owner,"trap",1) then
                inst:Remove()
                ...
                Remove_Items_By_Prefab(owner,"birdtrap",3)
                Remove_Items_By_Prefab(owner,"trap",1)

                local item = nil

                item = SpawnPrefab("meat")
                item.components.stackable.stacksize = 8
                owner.components.inventory:GiveItem(item)

                item = SpawnPrefab("malbatross_feather")
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local birdtrap_flag,birdtrap_num = Has_Enough_Items(owner,"birdtrap",3)
                local trap_flag,trap_num = Has_Enough_Items(owner,"trap",1)

                birdtrap_num = math.clamp(birdtrap_num,0,3)
                trap_num = math.clamp(trap_num,0,1)

                inst.__birdtrap_num:set(birdtrap_num)
                inst.__trap_num:set(trap_num)

                if birdtrap_num >= 3 and trap_num >= 1 then
...
return Prefab("hoshino_mission_white_26", fn, assets)