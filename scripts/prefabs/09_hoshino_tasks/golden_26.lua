...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_golden_26","hoshino_mission_golden_26")
        inst:ListenForEvent("hoshino_mission_golden_26",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_26_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_golden_26",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_26_board.tex"))
        ------- 任务描述
    ...
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 5 then
                ...
                owner.components.inventory:GiveItem(SpawnPrefab("hoshino_food_energy_drink"))
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            --- 排箫
            inst:ListenForEvent("hoshino_event.panflute",function(_,_table)
                ------------------------------------------------------------------------------
                --- 单个吹箫目标
                    local target = _table and _table.target
                    if target and target.sg and not target.sg:HasStateTag("sleeping") then
                        inst:DoTaskInTime(3,function()
                            if target and target:IsValid() and target.sg:HasStateTag("sleeping") then
                ...
                ------------------------------------------------------------------------------
                --- 重置计数器
                    if inst.components.hoshino_data:Add("num",0,0,5) < 5 then
...
return Prefab("hoshino_mission_golden_26", fn, assets)