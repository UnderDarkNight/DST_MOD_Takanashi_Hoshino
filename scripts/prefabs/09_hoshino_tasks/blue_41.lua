...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_blue_41","hoshino_mission_blue_41")
        inst:ListenForEvent("hoshino_mission_blue_41",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_41_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            local update_fn = function()
                ...
                display_text:SetString(""..num.."/3")
            end
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_blue_41",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_41_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 3 then
                inst:Remove()
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            inst:ListenForEvent("hoshino_event.waterballoon_explode",function(_,_table)
                if not (owner.components.hoshino_com_tag_sys and owner.components.hoshino_com_tag_sys:HasTag("hoshino_spell_type_swimming") ) then
                    return
                end
                local ents = _table and _table.ents
                local hit_player_flag = false
                for k,v in pairs(ents) do
                    if v and v:IsValid() and v:HasTag("player") and not v:HasTag("playerghost") then
                        hit_player_flag = true
                        break
                    end
                end
                if hit_player_flag then
                    local num = inst.components.hoshino_data:Add("num",1,0,3)
                    inst.__num:set(num)
                    if num >= 3 then
                        owner:PushEvent("hoshino_event.pad_warnning","main_page")
                    end
...
return Prefab("hoshino_mission_blue_41", fn, assets)