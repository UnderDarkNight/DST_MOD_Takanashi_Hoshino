...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_golden_27","hoshino_mission_golden_27")
        inst:ListenForEvent("hoshino_mission_golden_27",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_27_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_golden_27",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_27_board.tex"))
        ------- 任务描述
    ...
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 20 then
                ...
                item.components.stackable.stacksize = 15
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            inst.__reset_task = nil
            local function reset_count()
                if inst.components.hoshino_data:Add("num",0,0,20) < 20 then
            ...
            inst:ListenForEvent("hoshino_event.gunpowder_explode",function(_,gunpowder)
                ------------------------------------------------------------------------------
                ---
                    inst:DoTaskInTime(1,function()
                        local num = inst.components.hoshino_data:Add("num",GetStack(gunpowder),0,20)
                        inst.__num:set(num)
                        if num >= 20 then
                            owner:PushEvent("hoshino_event.pad_warnning","main_page")
                        end
                ...
                --- 重置计数器
                    reset_count()
                    -- if inst.__reset_task then
                    --     inst.__reset_task:Cancel()
                    --     inst.__reset_task = nil
                    -- else
                    --     inst.__reset_task = inst:DoTaskInTime(2,function()
                    --         reset_count()
                    --         inst.__reset_task = nil
                    --     end)
                    -- end
...
return Prefab("hoshino_mission_golden_27", fn, assets)