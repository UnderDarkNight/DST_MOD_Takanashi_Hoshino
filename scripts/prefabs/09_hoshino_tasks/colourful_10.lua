...
--- 专属参数
    ...
    local MISSION_REQUIRE_PREFAB = "deerclops_eyeball"
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 
...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_colourful_10","hoshino_mission_colourful_10")
        inst:ListenForEvent("hoshino_mission_colourful_10",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_colourful_10",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
return Prefab("hoshino_mission_colourful_10", fn, assets)