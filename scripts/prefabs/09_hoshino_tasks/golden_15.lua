...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_golden_15","hoshino_mission_golden_15")
        inst:ListenForEvent("hoshino_mission_golden_15",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_15_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_golden_15",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_15_board.tex"))
        ------- 任务描述
    ...
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 1 then
                ...
                -- item = SpawnPrefab("hoshino_food_mandrake_concentrate")
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            inst:DoTaskInTime(TUNING.HOSHINO_DEBUGGING_MODE and 5 or 15,function()
                if not inst.components.hoshino_data:Get("boss_spawned") then
                        ...
                        if type(pt) == type(Vector3(0,0,0)) then
                            local boss = SpawnPrefab("mutatedbearger")
                            boss.Transform:SetPosition(pt.x,0,pt.z)
                            local test_num = 100
                            local debuff_prefab = "hoshino_mission_golden_15_debuff"
                            while test_num > 0 do
            ...
            --- 击杀事件
            inst:ListenForEvent("hoshino_mission_golden_15_kill_with_debuff",function()
...
local function debuff_fn()
    ...
    inst.components.debuff:SetAttachedFn(function(inst,target) -- 玩家得到 debuff 的瞬间。 穿越洞穴、重新进存档 也会执行。
        ...
        -----------------------------------------------------
        --- 40%基础伤害减免
            target.components.combat.externaldamagetakenmultipliers:SetModifier(inst, 0.6)
        -----------------------------------------------------
        --- 2倍基础攻击
...
return Prefab("hoshino_mission_golden_15", fn, assets),
    Prefab("hoshino_mission_golden_15_debuff", debuff_fn, assets)