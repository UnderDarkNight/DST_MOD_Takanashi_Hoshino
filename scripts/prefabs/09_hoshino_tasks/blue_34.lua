...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_blue_34","hoshino_mission_blue_34")
        inst:ListenForEvent("hoshino_mission_blue_34",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_34_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_blue_34",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_34_board.tex"))
        ------- 任务描述
    ...
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 1 then
                ...
                owner.components.inventory:GiveItem(SpawnPrefab("leif_idol"))
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            local function check_player_axe_equipping(player)
                local weapon = player.components.combat:GetWeapon()
                if weapon and weapon.components.tool and weapon.components.tool:CanDoAction(ACTIONS.CHOP) then
                    return true
                end
                return false
            end
            inst:ListenForEvent("killed",function(_,_table) --- 广播来的事件
                ...
                if check_player_axe_equipping(owner) and prefab == "leif" or prefab == "leif_sparse" then
                    local num = inst.components.hoshino_data:Add("num",num or 1,0,1)
                    inst.__num:set(num)
                    if num >= 1 then
                        owner:PushEvent("hoshino_event.pad_warnning","main_page")
                    end
...
return Prefab("hoshino_mission_blue_34", fn, assets)