...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_golden_08","hoshino_mission_golden_08")
        inst:ListenForEvent("hoshino_mission_golden_08",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_08_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_golden_08",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_08_board.tex"))
        ------- 任务描述
    ...
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 1 then
                ...
                local exp = current_max_exp*0.80 -- 80% 经验
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            local punchingbag_prefabs = {
                ["punchingbag"] = true,
                ["punchingbag_shadow"] = true,
                ["punchingbag_lunar"] = true,
            }
            local function Get_Last_4_From_Table(_table)
                -- 初始化返回值为0
                local ret_1, ret_2, ret_3, ret_4 = 0, 0, 0, 0                
                -- 计算表中元素的数量
                local count = #_table                
                -- 根据表中的实际元素数量，从后向前获取最多4个元素
                if count > 0 then ret_1 = _table[count] end
                if count > 1 then ret_2 = _table[count - 1] end
                if count > 2 then ret_3 = _table[count - 2] end
                if count > 3 then ret_4 = _table[count - 3] end                
                -- 返回获取到的元素
...
return Prefab("hoshino_mission_golden_08", fn, assets)