...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__yellowamulet_num = net_uint(inst.GUID, "hoshino_mission_blue_15.yellowamulet","hoshino_mission_blue_15")
        inst.__orangeamulet_num = net_uint(inst.GUID, "hoshino_mission_blue_15.orangeamulet","hoshino_mission_blue_15")
        inst:ListenForEvent("hoshino_mission_blue_15",function()
            inst.yellowamulet_num = inst.__yellowamulet_num:value()
            inst.orangeamulet_num = inst.__orangeamulet_num:value()
        end)
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_15_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            yellowamulet_text:SetPosition(x,y)
            
            local orangeamulet_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            orangeamulet_text:SetPosition(x,y+delta_y)
        ...
        --- 检查任务是否完成
            local update_fn = function()
                ...
                local orangeamulet_flag,orangeamulet_num = Has_Enough_Items(ThePlayer,"orangeamulet",1)

                if yellowamulet_flag and orangeamulet_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                yellowamulet_num = math.clamp(yellowamulet_num,0,1)
                orangeamulet_num = math.clamp(orangeamulet_num,0,1)

                yellowamulet_text:SetString(""..yellowamulet_num.."/1")
                orangeamulet_text:SetString(""..orangeamulet_num.."/1")
            end
            update_fn()
            yellowamulet_text.inst:ListenForEvent("hoshino_mission_blue_15",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_15_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"yellowamulet",1) and Has_Enough_Items(owner,"orangeamulet",1) then
                inst:Remove()
                ...
                Remove_Items_By_Prefab(owner,"orangeamulet",1)
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local orangeamulet_flag,orangeamulet_num = Has_Enough_Items(owner,"orangeamulet",1)

                yellowamulet_num = math.clamp(yellowamulet_num,0,1)
                orangeamulet_num = math.clamp(orangeamulet_num,0,1)

                inst.__yellowamulet_num:set(yellowamulet_num)
                inst.__orangeamulet_num:set(orangeamulet_num)

                if yellowamulet_num >= 1 and orangeamulet_num >= 1 then
...
return Prefab("hoshino_mission_blue_15", fn, assets)