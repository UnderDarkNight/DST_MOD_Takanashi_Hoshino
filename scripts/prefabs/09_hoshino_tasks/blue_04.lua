...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_blue_04","hoshino_mission_blue_04")
        inst:ListenForEvent("hoshino_mission_blue_04",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_04_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_blue_04",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_04_board.tex"))
        ------- 任务描述
    ...
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 1 then
                ...
                -- print("获得经验",exp)
                owner.components.hoshino_com_level_sys:Exp_DoDelta(800)
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            inst:DoPeriodicTask(5,function()
                if inst.components.hoshino_data:Add("num",0) <= 0 
                    and owner.sg and owner.sg:HasStateTag("sleeping")
                    and owner.components.sanity.current <= 50
                    and owner.components.hunger.current <= 50
                    and owner.components.health.currenthealth <= 50
                    then
...
return Prefab("hoshino_mission_blue_04", fn, assets)