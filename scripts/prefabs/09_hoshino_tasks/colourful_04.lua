...
--- 专属参数
    ...
    local MISSION_ITEM_1 = "boss"
    local MISSION_ITEM_1_NUM = 2
    local MISSION_ITEM_2 = "alterguardianhat"
    local MISSION_ITEM_2_NUM = 1
    local MISSION_ITEM_3 = "thurible"
...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__boss_num = net_uint(inst.GUID, "hoshino_mission_colourful_04.boss","hoshino_mission_colourful_04")
        inst.__alterguardianhat_num = net_uint(inst.GUID, "hoshino_mission_colourful_04.alterguardianhat","hoshino_mission_colourful_04")
        inst.__thurible_num = net_uint(inst.GUID, "hoshino_mission_colourful_04.thurible","hoshino_mission_colourful_04")
        inst:ListenForEvent("hoshino_mission_colourful_04",function()
            inst.boss_num = inst.__boss_num:value()
            inst.alterguardianhat_num = inst.__alterguardianhat_num:value()
            inst.thurible_num = inst.__thurible_num:value()
        end)
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/colourful_mission.xml","colourful_mission_04_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local delta_y = -22
            local front_size = 25
            local boss_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            boss_text:SetPosition(x+40,y)

            local alterguardianhat_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            alterguardianhat_text:SetPosition(x,y+delta_y)

            local thurible_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            thurible_text:SetPosition(x,y+delta_y*2)
        --------------------------------------------------------------------------
        --- 检查任务是否完成
            local update_fn = function()

                local boss_num = inst.__boss_num:value()
                local boss_flag = boss_num >= MISSION_ITEM_1_NUM

                local alterguardianhat_flag,alterguardianhat_num = Has_Enough_Items(ThePlayer,"alterguardianhat",MISSION_ITEM_2_NUM)
                local thurible_flag,thurible_num = Has_Enough_Items(ThePlayer,"thurible",MISSION_ITEM_3_NUM)
                if boss_flag and alterguardianhat_flag and thurible_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                boss_num = math.clamp(boss_num,0,MISSION_ITEM_1_NUM)
                alterguardianhat_num = math.clamp(alterguardianhat_num,0,MISSION_ITEM_2_NUM)
                thurible_num = math.clamp(thurible_num,0,MISSION_ITEM_3_NUM)
                boss_text:SetString(""..boss_num.."/"..MISSION_ITEM_1_NUM)
                alterguardianhat_text:SetString(""..alterguardianhat_num.."/"..MISSION_ITEM_2_NUM)
                thurible_text:SetString(""..thurible_num.."/"..MISSION_ITEM_3_NUM)
            end
            update_fn()
            boss_text.inst:ListenForEvent("hoshino_mission_colourful_04",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/colourful_mission.xml","colourful_mission_04_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            local boss_num = inst.components.hoshino_data:Add("boss",0)
            if owner and boss_num >= MISSION_ITEM_1_NUM and Has_Enough_Items(owner,"alterguardianhat",MISSION_ITEM_2_NUM) and Has_Enough_Items(owner,"thurible",MISSION_ITEM_3_NUM) then
                inst:Remove()
                ...
                Remove_Items_By_Prefab(owner,"alterguardianhat",MISSION_ITEM_2_NUM)
                Remove_Items_By_Prefab(owner,"thurible",MISSION_ITEM_3_NUM)

                owner.components.inventory:GiveItem(SpawnPrefab("hoshino_item_blue_schist")) -- 给予物品
                owner.components.inventory:GiveItem(SpawnPrefab("hoshino_item_ether_essence")) -- 给予物品
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local alterguardianhat_flag,alterguardianhat_num = Has_Enough_Items(owner,"alterguardianhat",MISSION_ITEM_2_NUM)
                local thurible_flag,thurible_num = Has_Enough_Items(owner,"thurible",MISSION_ITEM_3_NUM)

                boss_num = math.clamp(boss_num,0,MISSION_ITEM_1_NUM)
                alterguardianhat_num = math.clamp(alterguardianhat_num,0,MISSION_ITEM_2_NUM)
                thurible_num = math.clamp(thurible_num,0,MISSION_ITEM_3_NUM)

                inst.__boss_num:set(boss_num)
                inst.__alterguardianhat_num:set(alterguardianhat_num)
                inst.__thurible_num:set(thurible_num)

                if boss_num >= MISSION_ITEM_1_NUM and alterguardianhat_num >= MISSION_ITEM_2_NUM and thurible_num >= MISSION_ITEM_3_NUM then
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            inst:ListenForEvent("hoshino_event.exp_broadcast",function(_,_table)
                ...
                local boss_data = inst.components.hoshino_data:Get("boss_data") or {}
                if prefab == "alterguardian_phase3" then
                    boss_data["alterguardian_phase3"] = 1
                elseif prefab == "stalker_atrium" then
                    boss_data["stalker_atrium"] = 1
                end
                inst.components.hoshino_data:Set("boss_data",boss_data)
                local boss_num = 0
                for _,v in pairs(boss_data) do
                    boss_num = boss_num + v
                end

                inst.components.hoshino_data:Set("boss",boss_num)
                inst.__boss_num:set(boss_num)
...
return Prefab("hoshino_mission_colourful_04", fn, assets)