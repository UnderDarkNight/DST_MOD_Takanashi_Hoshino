...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_golden_22","hoshino_mission_golden_22")
        inst:ListenForEvent("hoshino_mission_golden_22",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_22_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            local update_fn = function()
                ...
                display_text:SetString(""..num.."/400")
            end
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_golden_22",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_22_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 400 then
                inst:Remove()
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            inst:ListenForEvent("hoshino_event.exp_broadcast",function(_,_table)
                local num = inst.components.hoshino_data:Add("num",1,0,400)
                inst.__num:set(num)
                if num >= 400 then
                    owner:PushEvent("hoshino_event.pad_warnning","main_page")
                end
...
return Prefab("hoshino_mission_golden_22", fn, assets)