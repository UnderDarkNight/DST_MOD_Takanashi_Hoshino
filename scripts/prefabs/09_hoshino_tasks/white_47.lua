...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_white_47","hoshino_mission_white_47")
        inst:ListenForEvent("hoshino_mission_white_47",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/white_mission.xml","white_mission_47_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_white_47",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/white_mission.xml","white_mission_47_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 10 then
                inst:Remove()
                ...
                item =  SpawnPrefab("hoshino_item_12mm_shotgun_shells")
                item.components.stackable.stacksize = 3
                owner.components.inventory:GiveItem(item)

                local feather_prefabs = {"feather_robin_winter","feather_robin","feather_crow","feather_canary","goose_feather","malbatross_feather"}
                local ret_prefab = feather_prefabs[math.random(#feather_prefabs)]
                item =  SpawnPrefab(ret_prefab)
                -- item.components.stackable.stacksize = 3
                owner.components.inventory:GiveItem(item)
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            inst:ListenForEvent("hoshino_event.trade_2_target",function(_,_table)
...
return Prefab("hoshino_mission_white_47", fn, assets)