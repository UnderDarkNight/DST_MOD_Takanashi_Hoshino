...
--- 素材
    local assets =
    {
        Asset("IMAGE", "images/hoshino_mission/y_mission_b_38.tex"),
        Asset("ATLAS", "images/hoshino_mission/y_mission_b_38.xml"),
        Asset("IMAGE", "images/hoshino_mission/y_mission_s_38.tex"),
        Asset("ATLAS", "images/hoshino_mission/y_mission_s_38.xml"),
    }
...
--- 专属参数
    ...
    local MISSION_REQUIRE_NUM = TUNING.HOSHINO_DEBUGGING_MODE and 200 or 10000
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/y_mission_s_38.xml","y_mission_s_38.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---  91,112,136
            ...
            display_text:SetPosition(-300+50,-30)
        --------------------------------------------------------------------------
        --- 检查任务是否完成
...
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/y_mission_b_38.xml","y_mission_b_38.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add(MISSION_REQUIRE_PREFAB,0) >= MISSION_REQUIRE_NUM then
                ...
                if owner.components.hoshino_cards_sys then
                    owner.components.hoshino_cards_sys:AddRefreshNum(25)
                end
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            inst:ListenForEvent("hoshino_event.24_shop_recycled",function(_,_table)
                local coins = _table and _table.coins or 0
                inst.components.hoshino_data:Add(MISSION_REQUIRE_PREFAB,coins,0,MISSION_REQUIRE_NUM)
                mission_check()
            end,owner)
...
return Prefab("hoshino_mission_golden_38", fn, assets)