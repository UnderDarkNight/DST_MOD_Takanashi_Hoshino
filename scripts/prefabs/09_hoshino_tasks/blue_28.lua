...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__goldenpickaxe_num = net_uint(inst.GUID, "hoshino_mission_blue_28.goldenpickaxe","hoshino_mission_blue_28")
        inst.__goldenaxe_num = net_uint(inst.GUID, "hoshino_mission_blue_28.goldenaxe","hoshino_mission_blue_28")
        inst.__goldenshovel_num = net_uint(inst.GUID, "hoshino_mission_blue_28.goldenshovel","hoshino_mission_blue_28")
        inst:ListenForEvent("hoshino_mission_blue_28",function()
            inst.goldenpickaxe_num = inst.__goldenpickaxe_num:value()
            inst.goldenaxe_num = inst.__goldenaxe_num:value()
            inst.goldenshovel_num = inst.__goldenshovel_num:value()
        end)
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_28_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local goldenaxe_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            goldenaxe_text:SetPosition(x,y)

            local goldenpickaxe_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            goldenpickaxe_text:SetPosition(x,y+delta_y)

            local goldenshovel_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            goldenshovel_text:SetPosition(x,y+delta_y*2)
        --------------------------------------------------------------------------
        --- 检查任务是否完成
            local update_fn = function()
                local goldenpickaxe_flag,goldenpickaxe_num = Has_Enough_Items(ThePlayer,"goldenpickaxe",1)
                local goldenaxe_flag,goldenaxe_num = Has_Enough_Items(ThePlayer,"goldenaxe",1)
                local goldenshovel_flag,goldenshovel_num = Has_Enough_Items(ThePlayer,"goldenshovel",1)
                if goldenpickaxe_flag and goldenaxe_flag and goldenshovel_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                goldenpickaxe_num = math.clamp(goldenpickaxe_num,0,1)
                goldenaxe_num = math.clamp(goldenaxe_num,0,1)
                goldenshovel_num = math.clamp(goldenshovel_num,0,1)
                goldenpickaxe_text:SetString(""..goldenpickaxe_num.."/1")
                goldenaxe_text:SetString(""..goldenaxe_num.."/1")
                goldenshovel_text:SetString(""..goldenshovel_num.."/1")
            end
            update_fn()
            goldenpickaxe_text.inst:ListenForEvent("hoshino_mission_blue_28",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_28_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"goldenpickaxe",1) and Has_Enough_Items(owner,"goldenaxe",1) and Has_Enough_Items(owner,"goldenshovel",1) then
                inst:Remove()
                ...
                -- owner.components.hoshino_com_shop:CgoldenpickaxeitCoinDelta(800)

                Remove_Items_By_Prefab(owner,"goldenpickaxe",1)
                Remove_Items_By_Prefab(owner,"goldenaxe",1)
                Remove_Items_By_Prefab(owner,"goldenshovel",1)
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local goldenpickaxe_flag,goldenpickaxe_num = Has_Enough_Items(owner,"goldenpickaxe",1)
                local goldenaxe_flag,goldenaxe_num = Has_Enough_Items(owner,"goldenaxe",1)
                local goldenshovel_flag,goldenshovel_num = Has_Enough_Items(owner,"goldenshovel",1)

                goldenpickaxe_num = math.clamp(goldenpickaxe_num,0,1)
                goldenaxe_num = math.clamp(goldenaxe_num,0,1)
                goldenshovel_num = math.clamp(goldenshovel_num,0,1)

                inst.__goldenpickaxe_num:set(goldenpickaxe_num)
                inst.__goldenaxe_num:set(goldenaxe_num)
                inst.__goldenshovel_num:set(goldenshovel_num)

                if goldenpickaxe_num >= 1 and goldenaxe_num >= 1 and goldenshovel_num >= 1 then
...
local function fn()
    ...
    --- 
        ...
        inst.type = "gray"  -- "gray" "golden" "goldenaxe" "colourful" --- 给任务栏用的
    --------------------------------------------------------------------------------------------
    --- 数据组件
...
return Prefab("hoshino_mission_blue_28", fn, assets)