...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__watermelonhat_num = net_uint(inst.GUID, "hoshino_mission_blue_26.watermelonhat","hoshino_mission_blue_26")
        inst.__waterballoon_num = net_uint(inst.GUID, "hoshino_mission_blue_26.waterballoon","hoshino_mission_blue_26")
        inst:ListenForEvent("hoshino_mission_blue_26",function()
            inst.watermelonhat_num = inst.__watermelonhat_num:value()
            inst.waterballoon_num = inst.__waterballoon_num:value()
        end)
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_26_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local watermelonhat_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            watermelonhat_text:SetPosition(x,y)
            
            local waterballoon_text = bg:AddChild(Text(CODEFONT,front_size,"0/3",{ 91/255 , 112/255 ,136/255 , 1}))
            waterballoon_text:SetPosition(x,y+delta_y)

        --------------------------------------------------------------------------
        --- 检查任务是否完成
            local update_fn = function()
                local watermelonhat_flag,watermelonhat_num = Has_Enough_Items(ThePlayer,"watermelonhat",1)
                local waterballoon_flag,waterballoon_num = Has_Enough_Items(ThePlayer,"waterballoon",3)

                if watermelonhat_flag and waterballoon_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                watermelonhat_num = math.clamp(watermelonhat_num,0,1)
                waterballoon_num = math.clamp(waterballoon_num,0,3)

                watermelonhat_text:SetString(""..watermelonhat_num.."/1")
                waterballoon_text:SetString(""..waterballoon_num.."/3")
            end
            update_fn()
            watermelonhat_text.inst:ListenForEvent("hoshino_mission_blue_26",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_26_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"watermelonhat",1) and Has_Enough_Items(owner,"waterballoon",3) then
                inst:Remove()
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local watermelonhat_flag,watermelonhat_num = Has_Enough_Items(owner,"watermelonhat",1)
                local waterballoon_flag,waterballoon_num = Has_Enough_Items(owner,"waterballoon",3)

                watermelonhat_num = math.clamp(watermelonhat_num,0,1)
                waterballoon_num = math.clamp(waterballoon_num,0,3)

                inst.__watermelonhat_num:set(watermelonhat_num)
                inst.__waterballoon_num:set(waterballoon_num)

                if watermelonhat_num >= 1 and waterballoon_num >= 3 then
...
return Prefab("hoshino_mission_blue_26", fn, assets)