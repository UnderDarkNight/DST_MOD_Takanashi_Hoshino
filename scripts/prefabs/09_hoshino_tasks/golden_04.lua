...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_golden_04","hoshino_mission_golden_04")
        inst:ListenForEvent("hoshino_mission_golden_04",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_04_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_golden_04",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_04_board.tex"))
        ------- 任务描述
    ...
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 1 then
                ...
                local exp = current_max_exp*0.8 -- 5% 经验
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            --- 击杀事件
            inst:ListenForEvent("hoshino_mission_golden_04_kill_with_debuff",function()
        ...
        --- 死亡广播
            ...
            inst:ListenForEvent("entity_droploot",function(_,_table)
                if _table and _table.inst == target then
                    ...
                    local ents = TheSim:FindEntities(x,0, z, 100, {"player"})
                    for k, v in pairs(ents) do
                        v:PushEvent("hoshino_mission_golden_04_kill_with_debuff",target)
                    end
                end
            end,TheWorld)
            target.hoshino_mission_golden_04_debuff = true
        -----------------------------------------------------
        ---
...
return Prefab("hoshino_mission_golden_04", fn, assets),
    Prefab("hoshino_mission_golden_04_debuff", debuff_fn, assets)