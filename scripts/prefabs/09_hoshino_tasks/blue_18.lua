...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_blue_18","hoshino_mission_blue_18")
        inst:ListenForEvent("hoshino_mission_blue_18",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_18_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            local update_fn = function()
                ...
                display_text:SetString(""..num.."/35")
            end
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_blue_18",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_18_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 35 then
                inst:Remove()
                ...
                owner.components.inventory:GiveItem(SpawnPrefab("spidereggsack"))
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            --- 击杀
            inst:AddTag("hoshino_mission_blue_18")
            inst.killed_monsters = {}
            inst:ListenForEvent("hoshino_mission_blue_18_kill_with_debuff",function(_,target)
                if inst.killed_monsters[target] then
                    return
                end
                inst.killed_monsters[target] = true
                local num = inst.components.hoshino_data:Add("num",1,0,35)
                inst.__num:set(num)
                if num >= 35 then
                    owner:PushEvent("hoshino_event.pad_warnning","main_page")
                end
            ...
            --- 添加debuff
            local debuff_prefab = "hoshino_mission_blue_18_debuff"
            local function Add_Mission_Debuff(monster)
                local test_num = 10
                while test_num > 0 do
                ...
                print("error: add debuff failed",monster)
            end
            local function GetSpiderPrefabs()
                local ret_table = {}
                for i = 1, 20, 1 do
                    table.insert(ret_table,"spider_hider")
                end
                for i = 1, 10, 1 do
                    table.insert(ret_table,"spider_healer")
                end
                for i = 1, 5, 1 do
                    table.insert(ret_table,"spider_warrior")
                end
                return ret_table
            end
            local start_time_delay = TUNING.HOSHINO_DEBUGGING_MODE and 5 or 15            
            TheNet:Announce(tostring(start_time_delay).."秒后将有蜘蛛潮到达，请注意防护")
            inst:DoTaskInTime(start_time_delay,function()
                if not inst.components.hoshino_data:Get("spider_spawned") then
                    local spider_prefabs = GetSpiderPrefabs()
                    for i, prefab in ipairs(spider_prefabs) do
                        inst:DoTaskInTime(0.2*i,function()
                            ...
                            spider.Transform:SetPosition(random_pt.x,0,random_pt.z)
                            Add_Mission_Debuff(spider)
                            if i == #spider_prefabs then
...
return Prefab("hoshino_mission_blue_18", fn, assets),
    Prefab("hoshino_mission_blue_18_debuff", debuff_fn, assets)