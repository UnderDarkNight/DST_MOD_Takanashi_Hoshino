...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__goldnugget_num = net_uint(inst.GUID, "hoshino_mission_blue_23.goldnugget","hoshino_mission_blue_23")
        inst.__thulecite_num = net_uint(inst.GUID, "hoshino_mission_blue_23.thulecite","hoshino_mission_blue_23")
        inst:ListenForEvent("hoshino_mission_blue_23",function()
            inst.goldnugget_num = inst.__goldnugget_num:value()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_23_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local goldnugget_text = bg:AddChild(Text(CODEFONT,front_size,"0/20",{ 91/255 , 112/255 ,136/255 , 1}))
            goldnugget_text:SetPosition(x,y)
            
            local thulecite_text = bg:AddChild(Text(CODEFONT,front_size,"0/5",{ 91/255 , 112/255 ,136/255 , 1}))
            thulecite_text:SetPosition(x,y+delta_y)

        --------------------------------------------------------------------------
        --- 检查任务是否完成
            local update_fn = function()
                local goldnugget_flag,goldnugget_num = Has_Enough_Items(ThePlayer,"goldnugget",20)
                local thulecite_flag,thulecite_num = Has_Enough_Items(ThePlayer,"thulecite",5)

                if goldnugget_flag and thulecite_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                goldnugget_num = math.clamp(goldnugget_num,0,20)
                thulecite_num = math.clamp(thulecite_num,0,5)

                goldnugget_text:SetString(""..goldnugget_num.."/20")
                thulecite_text:SetString(""..thulecite_num.."/5")
            end
            update_fn()
            goldnugget_text.inst:ListenForEvent("hoshino_mission_blue_23",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_23_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"goldnugget",20) and Has_Enough_Items(owner,"thulecite",5) then
                inst:Remove()
                ...
                Remove_Items_By_Prefab(owner,"goldnugget",20)
                Remove_Items_By_Prefab(owner,"thulecite",5)
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local goldnugget_flag,goldnugget_num = Has_Enough_Items(owner,"goldnugget",20)
                local thulecite_flag,thulecite_num = Has_Enough_Items(owner,"thulecite",5)

                goldnugget_num = math.clamp(goldnugget_num,0,20)
                thulecite_num = math.clamp(thulecite_num,0,5)

                inst.__goldnugget_num:set(goldnugget_num)
                inst.__thulecite_num:set(thulecite_num)

                if goldnugget_num >= 20 and thulecite_num >= 5 then
...
return Prefab("hoshino_mission_blue_23", fn, assets)