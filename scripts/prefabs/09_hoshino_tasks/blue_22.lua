...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__heatrock_num = net_uint(inst.GUID, "hoshino_mission_blue_22.heatrock","hoshino_mission_blue_22")
        inst.__winterhat_num = net_uint(inst.GUID, "hoshino_mission_blue_22.winterhat","hoshino_mission_blue_22")
        inst:ListenForEvent("hoshino_mission_blue_22",function()
            inst.heatrock_num = inst.__heatrock_num:value()
            inst.winterhat_num = inst.__winterhat_num:value()
        end)
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_22_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local heatrock_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            heatrock_text:SetPosition(x,y)
            
            local winterhat_text = bg:AddChild(Text(CODEFONT,front_size,"0/2",{ 91/255 , 112/255 ,136/255 , 1}))
            winterhat_text:SetPosition(x,y+delta_y)

        --------------------------------------------------------------------------
        --- 检查任务是否完成
            local update_fn = function()
                local heatrock_flag,heatrock_num = Has_Enough_Items(ThePlayer,"heatrock",1)
                local winterhat_flag,winterhat_num = Has_Enough_Items(ThePlayer,"winterhat",2)

                if heatrock_flag and winterhat_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                heatrock_num = math.clamp(heatrock_num,0,1)
                winterhat_num = math.clamp(winterhat_num,0,2)

                heatrock_text:SetString(""..heatrock_num.."/1")
                winterhat_text:SetString(""..winterhat_num.."/2")
            end
            update_fn()
            heatrock_text.inst:ListenForEvent("hoshino_mission_blue_22",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_22_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"heatrock",1) and Has_Enough_Items(owner,"winterhat",2) then
                inst:Remove()
                ...
                Remove_Items_By_Prefab(owner,"heatrock",1)
                Remove_Items_By_Prefab(owner,"winterhat",2)
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local heatrock_flag,heatrock_num = Has_Enough_Items(owner,"heatrock",1)
                local winterhat_flag,winterhat_num = Has_Enough_Items(owner,"winterhat",2)

                heatrock_num = math.clamp(heatrock_num,0,1)
                winterhat_num = math.clamp(winterhat_num,0,2)

                inst.__heatrock_num:set(heatrock_num)
                inst.__winterhat_num:set(winterhat_num)

                if heatrock_num >= 1 and winterhat_num >= 2 then
...
return Prefab("hoshino_mission_blue_22", fn, assets)