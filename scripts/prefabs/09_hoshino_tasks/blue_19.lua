...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__transistor_num = net_uint(inst.GUID, "hoshino_mission_blue_19.transistor","hoshino_mission_blue_19")
        inst.__gears_num = net_uint(inst.GUID, "hoshino_mission_blue_19.gears","hoshino_mission_blue_19")
        inst:ListenForEvent("hoshino_mission_blue_19",function()
            inst.transistor_num = inst.__transistor_num:value()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_19_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local transistor_text = bg:AddChild(Text(CODEFONT,front_size,"0/5",{ 91/255 , 112/255 ,136/255 , 1}))
            transistor_text:SetPosition(x,y)
        ...
        --- 检查任务是否完成
            local update_fn = function()
                local transistor_flag,transistor_num = Has_Enough_Items(ThePlayer,"transistor",5)
                local gears_flag,gears_num = Has_Enough_Items(ThePlayer,"gears",10)

                if transistor_flag and gears_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                transistor_num = math.clamp(transistor_num,0,5)
                gears_num = math.clamp(gears_num,0,10)

                transistor_text:SetString(""..transistor_num.."/5")
                gears_text:SetString(""..gears_num.."/10")
            end
            update_fn()
            transistor_text.inst:ListenForEvent("hoshino_mission_blue_19",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_19_board.tex"))
        ------- 任务描述
    ...
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"transistor",5) and Has_Enough_Items(owner,"gears",10) then
                ...
                Remove_Items_By_Prefab(owner,"transistor",5)
                Remove_Items_By_Prefab(owner,"gears",10)
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local transistor_flag,transistor_num = Has_Enough_Items(owner,"transistor",5)
                local gears_flag,gears_num = Has_Enough_Items(owner,"gears",10)

                transistor_num = math.clamp(transistor_num,0,5)
                gears_num = math.clamp(gears_num,0,10)

                inst.__transistor_num:set(transistor_num)
                inst.__gears_num:set(gears_num)

                if transistor_num >= 5 and gears_num >= 10 then
...
return Prefab("hoshino_mission_blue_19", fn, assets)