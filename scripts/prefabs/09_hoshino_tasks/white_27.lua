...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__silk_num = net_uint(inst.GUID, "hoshino_mission_white_27.silk","hoshino_mission_white_27")
        inst.__pigskin_num = net_uint(inst.GUID, "hoshino_mission_white_27.pigskin","hoshino_mission_white_27")
        inst:ListenForEvent("hoshino_mission_white_27",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/white_mission.xml","white_mission_27_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local silk_text = bg:AddChild(Text(CODEFONT,front_size,"0/10",{ 91/255 , 112/255 ,136/255 , 1}))
            silk_text:SetPosition(x,y)
            
            local pigskin_text = bg:AddChild(Text(CODEFONT,front_size,"0/5",{ 91/255 , 112/255 ,136/255 , 1}))
            pigskin_text:SetPosition(x,y+delta_y)
        ...
        --- 检查任务是否完成
            local update_fn = function()
                local silk_flag,silk_num = Has_Enough_Items(ThePlayer,"silk",10)
                local pigskin_flag,pigskin_num = Has_Enough_Items(ThePlayer,"pigskin",5)

                if silk_flag and pigskin_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                silk_num = math.clamp(silk_num,0,10)
                pigskin_num = math.clamp(pigskin_num,0,5)

                silk_text:SetString(""..silk_num.."/10")
                pigskin_text:SetString(""..pigskin_num.."/5")
            end
            update_fn()
            silk_text.inst:ListenForEvent("hoshino_mission_white_27",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/white_mission.xml","white_mission_27_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"silk",10) and Has_Enough_Items(owner,"pigskin",5) then
                inst:Remove()
                ...
                Remove_Items_By_Prefab(owner,"silk",10)
                Remove_Items_By_Prefab(owner,"pigskin",5)

                owner.components.inventory:GiveItem(SpawnPrefab("orangegem"))
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local silk_flag,silk_num = Has_Enough_Items(owner,"silk",10)
                local pigskin_flag,pigskin_num = Has_Enough_Items(owner,"pigskin",5)
                ...
                if silk_num >= 10 and pigskin_num >= 5 then
...
return Prefab("hoshino_mission_white_27", fn, assets)