...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__oar_driftwood_num = net_uint(inst.GUID, "hoshino_mission_white_24.oar_driftwood","hoshino_mission_white_24")
        inst.__boat_grass_item_num = net_uint(inst.GUID, "hoshino_mission_white_24.boat_grass_item","hoshino_mission_white_24")
        inst:ListenForEvent("hoshino_mission_white_24",function()
            inst.oar_driftwood_num = inst.__oar_driftwood_num:value()
            inst.boat_grass_item_num = inst.__boat_grass_item_num:value()
        end)
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/white_mission.xml","white_mission_24_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local boat_grass_item_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            boat_grass_item_text:SetPosition(x,y)

            local oar_driftwood_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            oar_driftwood_text:SetPosition(x,y+delta_y)

        --------------------------------------------------------------------------
        --- 检查任务是否完成
            local update_fn = function()
                local oar_driftwood_flag,oar_driftwood_num = Has_Enough_Items(ThePlayer,"oar_driftwood",1)
                local boat_grass_item_flag,boat_grass_item_num = Has_Enough_Items(ThePlayer,"boat_grass_item",1)

                if oar_driftwood_flag and boat_grass_item_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                oar_driftwood_num = math.clamp(oar_driftwood_num,0,1)
                boat_grass_item_num = math.clamp(boat_grass_item_num,0,1)

                oar_driftwood_text:SetString(""..oar_driftwood_num.."/1")
                boat_grass_item_text:SetString(""..boat_grass_item_num.."/1")
            end
            update_fn()
            oar_driftwood_text.inst:ListenForEvent("hoshino_mission_white_24",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/white_mission.xml","white_mission_24_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"oar_driftwood",1) and Has_Enough_Items(owner,"boat_grass_item",1) then
                inst:Remove()
                ...
                Remove_Items_By_Prefab(owner,"oar_driftwood",1)
                Remove_Items_By_Prefab(owner,"boat_grass_item",1)
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local oar_driftwood_flag,oar_driftwood_num = Has_Enough_Items(owner,"oar_driftwood",1)
                local boat_grass_item_flag,boat_grass_item_num = Has_Enough_Items(owner,"boat_grass_item",1)

                oar_driftwood_num = math.clamp(oar_driftwood_num,0,1)
                boat_grass_item_num = math.clamp(boat_grass_item_num,0,1)

                inst.__oar_driftwood_num:set(oar_driftwood_num)
                inst.__boat_grass_item_num:set(boat_grass_item_num)

                if oar_driftwood_num >= 1 and boat_grass_item_num >= 1 then
...
return Prefab("hoshino_mission_white_24", fn, assets)