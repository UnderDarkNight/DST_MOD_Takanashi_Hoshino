...
--- 
    ...
    local function IsMushroom(item)
        if item and not has_cooked_field(item.prefab) and Mushroom_Prefabs[item.prefab] then
            return true
        end
        return false
    end
    local function Has_Mushroom(owner,num)
        local count_num = 0
        local ret_mushroom_inst = {}

        --- 玩家身上
        local inv_slots = owner.replica.inventory:GetItems()
        for i = 1, 50, 1 do
            if IsMushroom(inv_slots[i]) and ret_mushroom_inst[inv_slots[i]] == nil then
                count_num = count_num + GetStackNum(inv_slots[i])
                ret_mushroom_inst[inv_slots[i]] = true
            end
        end
        --- 遍历打开的背包
        ...
        for temp_container_inst, flag in pairs(openning_containers) do
            if temp_container_inst and flag and temp_container_inst.replica.inventoryitem and temp_container_inst.replica.container:IsOpenedBy(owner) then
                ...
                for i = 1, max_slots_num, 1 do
                    ...
                    if IsMushroom(temp_item) and ret_mushroom_inst[temp_item] == nil then
                        count_num = count_num + GetStackNum(temp_item)
                        ret_mushroom_inst[temp_item] = true
                    end
                end
            end
        end
        --- 返回计算结果
        return count_num >= num,ret_mushroom_inst
    end
    local function Remove_Mushroom(owner,num)
        local enough_flag, ret_mushroom_inst = Has_Mushroom(owner,num)
        if not enough_flag then
            return false
        end
        for temp_item, flag in pairs(ret_mushroom_inst) do
            if num <= 0 then
                break
            end
...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_white_30","hoshino_mission_white_30")
        inst:ListenForEvent("hoshino_mission_white_30",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/white_mission.xml","white_mission_30_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_white_30",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/white_mission.xml","white_mission_30_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Mushroom(owner,15) then
                inst:Remove()
        ...
        local function mission_check()
            ...
            if owner then
                local enough_flag,item_list = Has_Mushroom(owner,15)
                local item_num = 0
...
return Prefab("hoshino_mission_white_30", fn, assets)