...
--- 素材
    local assets =
    {
        ...
        Asset("IMAGE", "images/hoshino_mission/b_mission_b_25.tex"),
        Asset("ATLAS", "images/hoshino_mission/b_mission_b_25.xml"),
        Asset("IMAGE", "images/hoshino_mission/b_mission_s_25.tex"),
        Asset("ATLAS", "images/hoshino_mission/b_mission_s_25.xml"),
    }
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 专属参数
    local MISSION_REQUIRE_NUM = 6
    local MISSION_REQUIRE_PREFAB = "amulet"
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 
...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_blue_25","hoshino_mission_blue_25")
        inst:ListenForEvent("hoshino_mission_blue_25",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/b_mission_s_25.xml","b_mission_s_25.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_blue_25",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/b_mission_b_25.xml","b_mission_b_25.tex"))
        ------- 任务描述
    ...
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,MISSION_REQUIRE_PREFAB,MISSION_REQUIRE_NUM) then
                ...
                -- SpawnPrefab("mushroom_light2").Transform:SetPosition(owner.Transform:GetWorldPosition())
            end
        end)
...
return Prefab("hoshino_mission_blue_25", fn, assets)