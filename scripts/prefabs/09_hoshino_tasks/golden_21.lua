...
--- 专属参数
    ...
    local MISSION_ITEM_1 = "cutstone"
    local MISSION_ITEM_1_NUM = 40
    local MISSION_ITEM_2 = "boards"
    local MISSION_ITEM_2_NUM = 40
    local MISSION_ITEM_3 = "moonrocknugget"
    local MISSION_ITEM_3_NUM = 20
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 
...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__cutstone_num = net_uint(inst.GUID, "hoshino_mission_golden_21.cutstone","hoshino_mission_golden_21")
        inst.__boards_num = net_uint(inst.GUID, "hoshino_mission_golden_21.boards","hoshino_mission_golden_21")
        inst.__moonrocknugget_num = net_uint(inst.GUID, "hoshino_mission_golden_21.moonrocknugget","hoshino_mission_golden_21")
        inst:ListenForEvent("hoshino_mission_golden_21",function()
            inst.cutstone_num = inst.__cutstone_num:value()
            inst.boards_num = inst.__boards_num:value()
            inst.moonrocknugget_num = inst.__moonrocknugget_num:value()
        end)
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_21_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local delta_y = -22
            local front_size = 25
            local cutstone_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            cutstone_text:SetPosition(x,y)

            local boards_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            boards_text:SetPosition(x,y+delta_y)

            local moonrocknugget_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            moonrocknugget_text:SetPosition(x,y+delta_y*2)
        --------------------------------------------------------------------------
        --- 检查任务是否完成
            local update_fn = function()
                local cutstone_flag,cutstone_num = Has_Enough_Items(ThePlayer,"cutstone",MISSION_ITEM_1_NUM)
                local boards_flag,boards_num = Has_Enough_Items(ThePlayer,"boards",MISSION_ITEM_2_NUM)
                local moonrocknugget_flag,moonrocknugget_num = Has_Enough_Items(ThePlayer,"moonrocknugget",MISSION_ITEM_3_NUM)
                if cutstone_flag and boards_flag and moonrocknugget_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                cutstone_num = math.clamp(cutstone_num,0,MISSION_ITEM_1_NUM)
                boards_num = math.clamp(boards_num,0,MISSION_ITEM_2_NUM)
                moonrocknugget_num = math.clamp(moonrocknugget_num,0,MISSION_ITEM_3_NUM)
                cutstone_text:SetString(""..cutstone_num.."/"..MISSION_ITEM_1_NUM)
                boards_text:SetString(""..boards_num.."/"..MISSION_ITEM_2_NUM)
                moonrocknugget_text:SetString(""..moonrocknugget_num.."/"..MISSION_ITEM_3_NUM)
            end
            update_fn()
            cutstone_text.inst:ListenForEvent("hoshino_mission_golden_21",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_21_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"cutstone",MISSION_ITEM_1_NUM) and Has_Enough_Items(owner,"boards",MISSION_ITEM_2_NUM) and Has_Enough_Items(owner,"moonrocknugget",MISSION_ITEM_3_NUM) then
                inst:Remove()
                ...
                owner.components.hoshino_com_shop:CreditCoinDelta(1400)

                Remove_Items_By_Prefab(owner,"cutstone",MISSION_ITEM_1_NUM)
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local cutstone_flag,cutstone_num = Has_Enough_Items(owner,"cutstone",MISSION_ITEM_1_NUM)
                local boards_flag,boards_num = Has_Enough_Items(owner,"boards",MISSION_ITEM_2_NUM)
                local moonrocknugget_flag,moonrocknugget_num = Has_Enough_Items(owner,"moonrocknugget",MISSION_ITEM_3_NUM)

                cutstone_num = math.clamp(cutstone_num,0,MISSION_ITEM_1_NUM)
                boards_num = math.clamp(boards_num,0,MISSION_ITEM_2_NUM)
                moonrocknugget_num = math.clamp(moonrocknugget_num,0,MISSION_ITEM_3_NUM)

                inst.__cutstone_num:set(cutstone_num)
                inst.__boards_num:set(boards_num)
                inst.__moonrocknugget_num:set(moonrocknugget_num)

                if cutstone_num >= MISSION_ITEM_1_NUM and boards_num >= MISSION_ITEM_2_NUM and moonrocknugget_num >= MISSION_ITEM_3_NUM then
...
return Prefab("hoshino_mission_golden_21", fn, assets)