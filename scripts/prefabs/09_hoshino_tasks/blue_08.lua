...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__lightbulb_num = net_uint(inst.GUID, "hoshino_mission_blue_08.lightbulb","hoshino_mission_blue_08")
        inst.__rope_num = net_uint(inst.GUID, "hoshino_mission_blue_08.rope","hoshino_mission_blue_08")
        inst.__twigs_num = net_uint(inst.GUID, "hoshino_mission_blue_08.twigs","hoshino_mission_blue_08")
        inst:ListenForEvent("hoshino_mission_blue_08",function()
            inst.lightbulb_num = inst.__lightbulb_num:value()
            inst.rope_num = inst.__rope_num:value()
            inst.twigs_num = inst.__twigs_num:value()
        end)
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_08_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local delta_y = -22
            local front_size = 25
            local lightbulb_text = bg:AddChild(Text(CODEFONT,front_size,"0/2",{ 91/255 , 112/255 ,136/255 , 1}))
            lightbulb_text:SetPosition(x,y)

            local rope_text = bg:AddChild(Text(CODEFONT,front_size,"0/2",{ 91/255 , 112/255 ,136/255 , 1}))
            rope_text:SetPosition(x,y+delta_y)

            local twigs_text = bg:AddChild(Text(CODEFONT,front_size,"0/3",{ 91/255 , 112/255 ,136/255 , 1}))
            twigs_text:SetPosition(x,y+delta_y*2)
        --------------------------------------------------------------------------
        --- 检查任务是否完成
            local update_fn = function()
                local lightbulb_flag,lightbulb_num = Has_Enough_Items(ThePlayer,"lightbulb",2)
                local rope_flag,rope_num = Has_Enough_Items(ThePlayer,"rope",2)
                local twigs_flag,twigs_num = Has_Enough_Items(ThePlayer,"twigs",3)
                if lightbulb_flag and rope_flag and twigs_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                lightbulb_num = math.clamp(lightbulb_num,0,2)
                rope_num = math.clamp(rope_num,0,2)
                twigs_num = math.clamp(twigs_num,0,3)
                lightbulb_text:SetString(""..lightbulb_num.."/2")
                rope_text:SetString(""..rope_num.."/2")
                twigs_text:SetString(""..twigs_num.."/3")
            end
            update_fn()
            lightbulb_text.inst:ListenForEvent("hoshino_mission_blue_08",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_08_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"lightbulb",2) and Has_Enough_Items(owner,"rope",2) and Has_Enough_Items(owner,"twigs",3) then
                inst:Remove()
                ...
                -- owner.components.hoshino_com_shop:ClightbulbitCoinDelta(800)

                Remove_Items_By_Prefab(owner,"lightbulb",2)
                Remove_Items_By_Prefab(owner,"rope",2)
                Remove_Items_By_Prefab(owner,"twigs",3)
                ...
                owner.components.inventory:GiveItem(SpawnPrefab("lantern")) -- 给予物品
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local lightbulb_flag,lightbulb_num = Has_Enough_Items(owner,"lightbulb",2)
                local rope_flag,rope_num = Has_Enough_Items(owner,"rope",2)
                local twigs_flag,twigs_num = Has_Enough_Items(owner,"twigs",3)

                lightbulb_num = math.clamp(lightbulb_num,0,2)
                rope_num = math.clamp(rope_num,0,2)
                twigs_num = math.clamp(twigs_num,0,3)

                inst.__lightbulb_num:set(lightbulb_num)
                inst.__rope_num:set(rope_num)
                inst.__twigs_num:set(twigs_num)

                if lightbulb_num >= 2 and rope_num >= 2 and twigs_num >= 3 then
...
local function fn()
    ...
    --- 
        ...
        inst.type = "gray"  -- "gray" "golden" "rope" "colourful" --- 给任务栏用的
    --------------------------------------------------------------------------------------------
    --- 数据组件
...
return Prefab("hoshino_mission_blue_08", fn, assets)