...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_golden_30","hoshino_mission_golden_30")
        inst:ListenForEvent("hoshino_mission_golden_30",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_30_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_golden_30",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_30_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            local buff_prefabs = {
                ["buff_electricattack"] = true,     -- 福特羊肉冻 buff
                ["buff_attack"] = true,             -- 辣椒粉 buff
                ["buff_playerabsorption"] = true,   -- 大蒜粉 buff
                ["buff_workeffectiveness"] = true,  -- 蜂蜜 buff
                ["healthregenbuff"] = true,         -- 彩虹糖豆 buff
            }
            local function food_buff_checker()
                ...
                inst:DoTaskInTime(1,function()
                    local num = 0
                    for debuff_prefab,flag in pairs(buff_prefabs) do
                        if owner:HasDebuff(debuff_prefab) then
                            num = num + 1
                        end
                    end
                    ...
                    if inst.components.hoshino_data:Add("num",0) < 5 then
                        ...
                        inst.components.hoshino_data:Set("num",5)
                        inst.__num:set(5)
                    end
            ...
            inst:ListenForEvent("oneat",food_buff_checker,owner)
            inst:DoPeriodicTask(1,food_buff_checker)
        end)
...
return Prefab("hoshino_mission_golden_30", fn, assets)