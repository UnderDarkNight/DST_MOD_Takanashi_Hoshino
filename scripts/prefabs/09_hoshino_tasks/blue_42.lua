...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_blue_42","hoshino_mission_blue_42")
        inst:ListenForEvent("hoshino_mission_blue_42",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_42_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_blue_42",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_42_board.tex"))
        ------- 任务描述
    ...
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 1 then
                ...
                local exp = current_max_exp*0.20 -- 25% 经验
                ...
                local reward = inst.components.hoshino_data:Get("reward") or "axe"
                owner.components.inventory:GiveItem(SpawnPrefab(reward))
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            inst:ListenForEvent("hoshino_event.exp_broadcast",function(_,_table)
                ...
                if target and target:HasTag("epic") and target:GetNearestPlayer(true) == owner then
                    local weapon = owner.components.combat:GetWeapon()
                    if weapon and weapon.components.tool then
                        inst.components.hoshino_data:Set("reward",weapon.prefab or "axe")
...
return Prefab("hoshino_mission_blue_42", fn, assets)