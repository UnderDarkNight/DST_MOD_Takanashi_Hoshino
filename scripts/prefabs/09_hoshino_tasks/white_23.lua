...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__spear_num = net_uint(inst.GUID, "hoshino_mission_white_23.spear","hoshino_mission_white_23")
        inst.__flint_num = net_uint(inst.GUID, "hoshino_mission_white_23.flint","hoshino_mission_white_23")
        inst:ListenForEvent("hoshino_mission_white_23",function()
            inst.spear_num = inst.__spear_num:value()
            inst.flint_num = inst.__flint_num:value()
        end)
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/white_mission.xml","white_mission_23_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local spear_text = bg:AddChild(Text(CODEFONT,front_size,"0/2",{ 91/255 , 112/255 ,136/255 , 1}))
            spear_text:SetPosition(x,y)
            
            local flint_text = bg:AddChild(Text(CODEFONT,front_size,"0/2",{ 91/255 , 112/255 ,136/255 , 1}))
            flint_text:SetPosition(x,y+delta_y)
        ...
        --- 检查任务是否完成
            local update_fn = function()
                local spear_flag,spear_num = Has_Enough_Items(ThePlayer,"spear",2)
                local flint_flag,flint_num = Has_Enough_Items(ThePlayer,"flint",2)

                if spear_flag and flint_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                spear_num = math.clamp(spear_num,0,2)
                flint_num = math.clamp(flint_num,0,2)

                spear_text:SetString(""..spear_num.."/2")
                flint_text:SetString(""..flint_num.."/2")
            end
            update_fn()
            spear_text.inst:ListenForEvent("hoshino_mission_white_23",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/white_mission.xml","white_mission_23_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"spear",2) and Has_Enough_Items(owner,"flint",2) then
                inst:Remove()
                ...
                Remove_Items_By_Prefab(owner,"spear",2)
                Remove_Items_By_Prefab(owner,"flint",2)
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local spear_flag,spear_num = Has_Enough_Items(owner,"spear",2)
                local flint_flag,flint_num = Has_Enough_Items(owner,"flint",2)

                spear_num = math.clamp(spear_num,0,2)
                flint_num = math.clamp(flint_num,0,2)

                inst.__spear_num:set(spear_num)
                inst.__flint_num:set(flint_num)

                if spear_num >= 2 and flint_num >= 2 then
...
return Prefab("hoshino_mission_white_23", fn, assets)