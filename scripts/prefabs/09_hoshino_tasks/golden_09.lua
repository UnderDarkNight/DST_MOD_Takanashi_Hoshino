...
--- 素材
    local assets =
    {
        ...
        Asset("IMAGE", "images/hoshino_mission/y_mission_b_09.tex"),
        Asset("ATLAS", "images/hoshino_mission/y_mission_b_09.xml"),
        Asset("IMAGE", "images/hoshino_mission/y_mission_s_09.tex"),
        Asset("ATLAS", "images/hoshino_mission/y_mission_s_09.xml"),
    }
...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_golden_09","hoshino_mission_golden_09")
        inst:ListenForEvent("hoshino_mission_golden_09",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/y_mission_s_09.xml","y_mission_s_09.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_golden_09",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/y_mission_b_09.xml","y_mission_b_09.tex"))
        ------- 任务描述
        ...
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 1 then
                ...
                --- 上debuff
                    ...
                    local test_num = 1000
                    local debuff_time = TUNING.HOSHINO_DEBUGGING_MODE and 30 or 10*60
                    while test_num > 0 do
                        local debuff_inst = owner:GetDebuff(debuff_prefab)
                        if debuff_inst and debuff_inst:IsValid() then
                            owner.components.hoshino_data:Add("hoshino_mission_golden_09_debuff",debuff_time,0,10000000)
                            break
                        end
    ...
    inst.components.debuff:SetAttachedFn(function(inst,target) -- 玩家得到 debuff 的瞬间。 穿越洞穴、重新进存档 也会执行。
        ...
        --
            inst:DoPeriodicTask(1,function()
                local time = target.components.hoshino_data:Add("hoshino_mission_golden_09_debuff",-1,0,10000000)
                -- print("time",time)
                if time == 0 then
                    inst:Remove()
                end
...
return Prefab("hoshino_mission_golden_09", fn, assets),
    Prefab("hoshino_mission_golden_09_debuff", debuff_fn, assets)