...
--- 素材
    local assets =
    {
        ...
        Asset("IMAGE", "images/hoshino_mission/r_mission_b_11.tex"),
        Asset("ATLAS", "images/hoshino_mission/r_mission_b_11.xml"),
        Asset("IMAGE", "images/hoshino_mission/r_mission_s_11.tex"),
        Asset("ATLAS", "images/hoshino_mission/r_mission_s_11.xml"),
    }
...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_colourful_11","hoshino_mission_colourful_11")
        inst:ListenForEvent("hoshino_mission_colourful_11",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/r_mission_s_11.xml","r_mission_s_11.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_colourful_11",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/r_mission_b_11.xml","r_mission_b_11.tex"))
        ------- 任务描述
    ...
    local function Task_Delivery_Event_Install(inst)
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            inst:DoPeriodicTask(1,function()
                ...
                if (owner:HasTag("playerghost") or owner.components.health:IsDead() )then
                    inst:Remove()
                    return
                end
                ...
                local time = inst.components.hoshino_data:Add("time",1,0,max_time)
            ...
            local function add_debuff_2_monster(monster)
                local debuff_prefab = "hoshino_mission_colourful_11_debuff"
                local test_num = 100
                while test_num > 0 do
            ...
            inst:DoPeriodicTask(4,function()
                ...
                local monster_prefabs = {
                        "crawlingnightmare","nightmarebeak","ruinsnightmare",
                ...
                local monster = SpawnPrefab(monster_prefabs[math.random(1,#monster_prefabs)])
                monster.Transform:SetPosition(pt.x,0,pt.z)
                add_debuff_2_monster(monster)

            end)
            --- 精神控制
            local function MindControlPlayer(player)
                for i = 1, 15, 1 do
                    inst:DoTaskInTime(0.5*i, function()
                        if inst:IsValid() and player and player:IsValid()
                            and not player:HasTag("playerghost")
                            and not (player.components.health and player.components.health:IsDead() )
                            and inst.components.hoshino_data:Add("num",0) == 0
                            then
            ...
            inst:DoPeriodicTask(30,function()
                ...
                local players = TheSim:FindEntities(x,0,z,100,{"player"},{"playerghost"})
                for _,player in ipairs(players) do
                    MindControlPlayer(player)
                end
            end)
...
return Prefab("hoshino_mission_colourful_11", fn, assets),
    Prefab("hoshino_mission_colourful_11_debuff", debuff_fn, assets)