...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 
    local MISSION_REQUIRE_NUM = TUNING.HOSHINO_DEBUGGING_MODE and 10000 or 10000
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_colourful_06","hoshino_mission_colourful_06")
        inst:ListenForEvent("hoshino_mission_colourful_06",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_colourful_06",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/colourful_mission.xml","colourful_mission_06_board.tex"))
        ------- 任务描述
    ...
    local function Task_Delivery_Event_Install(inst)
        ...
        inst:ListenForEvent("active",function(inst,owner)            
            local function mission_checker()
                local num = owner.components.hoshino_com_shop:GetCreditCoins()
                num = math.clamp(num,0,MISSION_REQUIRE_NUM)
                inst.__num:set(num)
...
return Prefab("hoshino_mission_colourful_06", fn, assets)