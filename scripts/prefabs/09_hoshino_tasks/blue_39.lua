...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
---
    local function GetFollowingBunnymen(player)
        if player.components.leader then
            local followers = player.components.leader.followers or {}
            local num = 0
            for monster, v in pairs(followers) do
                if type(monster) == "table" and monster:IsValid() and monster.prefab == "bunnyman" then
                    num = num + 1
                end
            end
            return num
...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_blue_39","hoshino_mission_blue_39")
        inst:ListenForEvent("hoshino_mission_blue_39",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_39_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_blue_39",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_39_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and GetFollowingBunnymen(owner) >= 4 then
                inst:Remove()
                ...
                owner.components.inventory:GiveItem(SpawnPrefab("earmuffshat"))
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            inst:DoPeriodicTask(5,function()
                local num = GetFollowingBunnymen(owner)
                inst.__num:set(num)
                if num >= 4 then
...
return Prefab("hoshino_mission_blue_39", fn, assets)