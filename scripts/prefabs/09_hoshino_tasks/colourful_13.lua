...
--- 素材
    local assets =
    {
        ...
        Asset("IMAGE", "images/hoshino_mission/r_mission_b_13.tex"),
        Asset("ATLAS", "images/hoshino_mission/r_mission_b_13.xml"),
        Asset("IMAGE", "images/hoshino_mission/r_mission_s_13.tex"),
        Asset("ATLAS", "images/hoshino_mission/r_mission_s_13.xml"),
    }
...
--- 专属参数
    ...
    local MISSION_ITEM_1 = "hoshino_item_nine"
...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__hoshino_item_nine_num = net_uint(inst.GUID, "hoshino_mission_colourful_13.hoshino_item_nine","hoshino_mission_colourful_13")
        inst.__hoshino_item_accessory_remnants_num = net_uint(inst.GUID, "hoshino_mission_colourful_13.hoshino_item_accessory_remnants","hoshino_mission_colourful_13")
        inst:ListenForEvent("hoshino_mission_colourful_13",function()
            inst.hoshino_item_nine_num = inst.__hoshino_item_nine_num:value()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/r_mission_s_13.xml","r_mission_s_13.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local hoshino_item_nine_text = bg:AddChild(Text(CODEFONT,front_size,"0/10",{ 91/255 , 112/255 ,136/255 , 1}))
            hoshino_item_nine_text:SetPosition(x,y)
        ...
        --- 检查任务是否完成
            local update_fn = function()
                local hoshino_item_nine_flag,hoshino_item_nine_num = Has_Enough_Items(ThePlayer,"hoshino_item_nine",MISSION_ITEM_1_NUM)
                ...
                if hoshino_item_nine_flag and hoshino_item_accessory_remnants_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                hoshino_item_nine_num = math.clamp(hoshino_item_nine_num,0,MISSION_ITEM_1_NUM)
                hoshino_item_accessory_remnants_num = math.clamp(hoshino_item_accessory_remnants_num,0,MISSION_ITEM_2_NUM)

                hoshino_item_nine_text:SetString(""..hoshino_item_nine_num.."/"..MISSION_ITEM_1_NUM)
                hoshino_item_accessory_remnants_text:SetString(""..hoshino_item_accessory_remnants_num.."/"..MISSION_ITEM_2_NUM)
            end
            update_fn()
            hoshino_item_nine_text.inst:ListenForEvent("hoshino_mission_colourful_13",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/r_mission_b_13.xml","r_mission_b_13.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"hoshino_item_nine",MISSION_ITEM_1_NUM) and Has_Enough_Items(owner,"hoshino_item_accessory_remnants",MISSION_ITEM_2_NUM) then
                inst:Remove()
                ...
                Remove_Items_By_Prefab(owner,"hoshino_item_nine",MISSION_ITEM_1_NUM)
                Remove_Items_By_Prefab(owner,"hoshino_item_accessory_remnants",MISSION_ITEM_2_NUM)

                local item = SpawnPrefab("hoshino_item_artifact_du")
                if item then
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local hoshino_item_nine_flag,hoshino_item_nine_num = Has_Enough_Items(owner,"hoshino_item_nine",MISSION_ITEM_1_NUM)
                ...
                inst.__hoshino_item_nine_num:set(hoshino_item_nine_num)
                inst.__hoshino_item_accessory_remnants_num:set(hoshino_item_accessory_remnants_num)

                if hoshino_item_nine_num >= MISSION_ITEM_1_NUM and hoshino_item_accessory_remnants_num >= MISSION_ITEM_2_NUM then
...
return Prefab("hoshino_mission_colourful_13", fn, assets)