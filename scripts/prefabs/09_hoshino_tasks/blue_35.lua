...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_blue_35","hoshino_mission_blue_35")
        inst:ListenForEvent("hoshino_mission_blue_35",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_35_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_blue_35",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_35_board.tex"))
        ------- 任务描述
    ...
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and inst.components.hoshino_data:Add("num",0) >= 1 then
                ...
                local exp = current_max_exp*0.25 -- 5% 经验
                ...
                owner.components.inventory:GiveItem(SpawnPrefab("gelblob_storage_kit"))
                -- SpawnPrefab("gelblob_storage").Transform:SetPosition(owner.Transform:GetWorldPosition())
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            inst:ListenForEvent("exit_gelblob",function(_,_table) --- 广播来的事件
...
return Prefab("hoshino_mission_blue_35", fn, assets)