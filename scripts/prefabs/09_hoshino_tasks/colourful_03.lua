...
--- 专属参数
    ...
    -- local MISSION_REQUIRE_PREFAB = "goldnugget"
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 
...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_colourful_03","hoshino_mission_colourful_03")
        inst:ListenForEvent("hoshino_mission_colourful_03",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/colourful_mission.xml","colourful_mission_03_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_colourful_03",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/colourful_mission.xml","colourful_mission_03_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            inst:ListenForEvent("hoshino_mission_colourful_03_kill_with_debuff",function()
            ...
            local function Add_debuff_2_Boss(target)
                local test_num = 100
                local debuff_prefab = "hoshino_mission_colourful_03_debuff"
                while test_num > 0 do
            ...
            inst:DoTaskInTime(TUNING.HOSHINO_DEBUGGING_MODE and 5 or 15,function()
                if not inst.components.hoshino_data:Get("boss_spawned") then
                    ...
                    local boss_inst = SpawnPrefab(boss_prefabs[math.random(#boss_prefabs)])
                    boss_inst.Transform:SetPosition(pt.x,0,pt.z)
                    Add_debuff_2_Boss(boss_inst)
...
local function debuff_fn()
    ...
    inst.components.debuff:SetAttachedFn(function(inst,target) -- 玩家得到 debuff 的瞬间。 穿越洞穴、重新进存档 也会执行。
        ...
        -----------------------------------------------------
        --- 60%减伤
            target.components.health.externalabsorbmodifiers:SetModifier(inst, 0.6)
        -----------------------------------------------------
        --- 5倍攻击
            target.components.combat.externaldamagemultipliers:SetModifier(inst, 5)
        -----------------------------------------------------
        --- 5倍血量
        ...
        --- 死亡广播
            target:ListenForEvent("minhealth",function()
                ...
                for k, v in pairs(ents) do
                    v:PushEvent("hoshino_mission_colourful_03_kill_with_debuff",target)
                end
            end)
        -----------------------------------------------------
        ---
...
return Prefab("hoshino_mission_colourful_03", fn, assets),
    Prefab("hoshino_mission_colourful_03_debuff", debuff_fn, assets)