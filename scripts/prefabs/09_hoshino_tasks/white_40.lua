...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__tophat_num = net_uint(inst.GUID, "hoshino_mission_white_40.tophat","hoshino_mission_white_40")
        inst.__bushhat_num = net_uint(inst.GUID, "hoshino_mission_white_40.bushhat","hoshino_mission_white_40")
        inst.__beehat_num = net_uint(inst.GUID, "hoshino_mission_white_40.beehat","hoshino_mission_white_40")
        inst:ListenForEvent("hoshino_mission_white_40",function()
            inst.tophat_num = inst.__tophat_num:value()
            inst.bushhat_num = inst.__bushhat_num:value()
            inst.beehat_num = inst.__beehat_num:value()
        end)
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/white_mission.xml","white_mission_40_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local delta_y = -22
            local front_size = 25
            local tophat_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            tophat_text:SetPosition(x,y)

            local bushhat_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            bushhat_text:SetPosition(x,y+delta_y)

            local beehat_text = bg:AddChild(Text(CODEFONT,front_size,"0/1",{ 91/255 , 112/255 ,136/255 , 1}))
            beehat_text:SetPosition(x,y+delta_y*2)
        --------------------------------------------------------------------------
        --- 检查任务是否完成
            local update_fn = function()
                local tophat_flag,tophat_num = Has_Enough_Items(ThePlayer,"tophat",1)
                local bushhat_flag,bushhat_num = Has_Enough_Items(ThePlayer,"bushhat",1)
                local beehat_flag,beehat_num = Has_Enough_Items(ThePlayer,"beehat",1)
                if tophat_flag and bushhat_flag and beehat_flag then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                tophat_num = math.clamp(tophat_num,0,1)
                bushhat_num = math.clamp(bushhat_num,0,1)
                beehat_num = math.clamp(beehat_num,0,1)
                tophat_text:SetString(""..tophat_num.."/1")
                bushhat_text:SetString(""..bushhat_num.."/1")
                beehat_text:SetString(""..beehat_num.."/1")
            end
            update_fn()
            tophat_text.inst:ListenForEvent("hoshino_mission_white_40",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/white_mission.xml","white_mission_40_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            if owner and Has_Enough_Items(owner,"tophat",1) and Has_Enough_Items(owner,"bushhat",1) and Has_Enough_Items(owner,"beehat",1) then
                inst:Remove()
                ...
                -- owner.components.hoshino_com_shop:CtophatitCoinDelta(800)

                Remove_Items_By_Prefab(owner,"tophat",1)
                Remove_Items_By_Prefab(owner,"bushhat",1)
                Remove_Items_By_Prefab(owner,"beehat",1)
        ...
        local function mission_check()
            ...
            if owner then
                ...
                local tophat_flag,tophat_num = Has_Enough_Items(owner,"tophat",1)
                local bushhat_flag,bushhat_num = Has_Enough_Items(owner,"bushhat",1)
                local beehat_flag,beehat_num = Has_Enough_Items(owner,"beehat",1)

                tophat_num = math.clamp(tophat_num,0,1)
                bushhat_num = math.clamp(bushhat_num,0,1)
                beehat_num = math.clamp(beehat_num,0,1)

                inst.__tophat_num:set(tophat_num)
                inst.__bushhat_num:set(bushhat_num)
                inst.__beehat_num:set(beehat_num)

                if tophat_num >= 1 and bushhat_num >= 1 and beehat_num >= 1 then
...
local function fn()
    ...
    --- 
        ...
        inst.type = "white"  -- "gray" "golden" "bushhat" "colourful" --- 给任务栏用的
    --------------------------------------------------------------------------------------------
    --- 数据组件
...
return Prefab("hoshino_mission_white_40", fn, assets)