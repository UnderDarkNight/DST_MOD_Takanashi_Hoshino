...
--- 专属参数
    ...
    local MISSION_ITEM_1 = "decor_pictureframe"
    local MISSION_ITEM_1_NUM = 1
    local MISSION_ITEM_2 = "featherpencil"
...
--- net install
    local function Net_Vars_Install(inst)
        ...
        inst.__decor_pictureframe_num = net_uint(inst.GUID, "hoshino_mission_blue_36.decor_pictureframe","hoshino_mission_blue_36")
        inst.__featherpencil_num = net_uint(inst.GUID, "hoshino_mission_blue_36.featherpencil","hoshino_mission_blue_36")
        inst:ListenForEvent("hoshino_mission_blue_36",function()
            inst.decor_pictureframe_num = inst.__decor_pictureframe_num:value()
            inst.featherpencil_num = inst.__featherpencil_num:value()
        end)
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_36_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        ---
            ...
            local decor_pictureframe_text = bg:AddChild(Text(CODEFONT,front_size,"0/10",{ 91/255 , 112/255 ,136/255 , 1}))
            decor_pictureframe_text:SetPosition(x,y)
            
            local featherpencil_text = bg:AddChild(Text(CODEFONT,front_size,"0/10",{ 91/255 , 112/255 ,136/255 , 1}))
            featherpencil_text:SetPosition(x,y+delta_y)
        ...
        --- 检查任务是否完成
            local update_fn = function()
                local decor_pictureframe_num = inst.__decor_pictureframe_num:value()
                local featherpencil_num = inst.__featherpencil_num:value()
                if decor_pictureframe_num + featherpencil_num >= 2 then
                    button_delivery:Show()
                else
                    button_delivery:Hide()
                end
                decor_pictureframe_num = math.clamp(decor_pictureframe_num,0,MISSION_ITEM_1_NUM)
                featherpencil_num = math.clamp(featherpencil_num,0,MISSION_ITEM_2_NUM)

                decor_pictureframe_text:SetString(""..decor_pictureframe_num.."/"..MISSION_ITEM_1_NUM)
                featherpencil_text:SetString(""..featherpencil_num.."/"..MISSION_ITEM_2_NUM)
            end
            update_fn()
            decor_pictureframe_text.inst:ListenForEvent("hoshino_mission_blue_36",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/blue_mission.xml","blue_mission_36_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        inst:ListenForEvent("task_delivery", function()
            ...
            local featherpencil_num =  inst.components.hoshino_data:Add("featherpencil",0)
            local decor_pictureframe_num =  inst.components.hoshino_data:Add("decor_pictureframe",0)

            if owner and featherpencil_num + decor_pictureframe_num >= 2 then
                inst:Remove()
                ...
                local exp = current_max_exp*0.30 -- 25% 经验
                owner.components.hoshino_com_level_sys:Exp_DoDelta(exp)
                -- owner.components.hoshino_com_shop:CreditCoinDelta(150)

                local gem_prefabs = {"redgem","orangegem","yellowgem","greengem",
                    "bluegem","purplegem","opalpreciousgem"}
                local ret_gem = gem_prefabs[math.random(#gem_prefabs)]
                owner.components.inventory:GiveItem(SpawnPrefab(ret_gem))
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            local function warnning_checker()
                local featherpencil_num =  inst.components.hoshino_data:Add("featherpencil",0)
                inst.__featherpencil_num:set(featherpencil_num)

                local decor_pictureframe_num =  inst.components.hoshino_data:Add("decor_pictureframe",0)
                inst.__decor_pictureframe_num:set(decor_pictureframe_num)

                if featherpencil_num + decor_pictureframe_num >= 2 then
            ...
            --- 绘画事件监听
            inst:ListenForEvent("hoshino_event.drawable_drawn",function(_,target)
                if target and target.prefab == "decor_pictureframe" then
                    local featherpencil_num =  inst.components.hoshino_data:Add("featherpencil",1,0,1)
                    warnning_checker()
                end
            end,owner)
            --- 制作画框
            inst:ListenForEvent("builditem",function(_,_table)
                if _table and _table.item and _table.item.prefab == "decor_pictureframe" then
                    local decor_pictureframe_num =  inst.components.hoshino_data:Add("decor_pictureframe",1,0,1)
                    warnning_checker()
                end
            end,owner)
...
return Prefab("hoshino_mission_blue_36", fn, assets)