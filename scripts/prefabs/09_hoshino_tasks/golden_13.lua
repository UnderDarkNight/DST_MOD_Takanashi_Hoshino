...
--- net install
    local function Net_Vars_Install(inst)
        inst.__num = net_uint(inst.GUID, "hoshino_mission_golden_13","hoshino_mission_golden_13")
        inst:ListenForEvent("hoshino_mission_golden_13",function()
...
--- 用于平板显示的API，返回Widget图像。client端调用
    ...
    local GetPadDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_13_pad.tex"))
        --------------------------------------------------------------------------
        --- 放弃按钮
        ...
        --- 检查任务是否完成
            ...
            update_fn()
            display_text.inst:ListenForEvent("hoshino_mission_golden_13",update_fn,inst)
        --------------------------------------------------------------------------
        return bg
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 用于任务栏显示的组件，返回Widget图像。client端调用
    local GetBoardDisplayBox = function(inst,box)
        local bg = box:AddChild(Image("images/hoshino_mission/golden_mission.xml","golden_mission_13_board.tex"))
        ------- 任务描述
...
--- 检查和提交
    local function Task_Delivery_Event_Install(inst)
        ...
        inst:ListenForEvent("active",function(inst,owner)
            ...
            TheNet:Announce(tostring(start_time_delay).."秒后将有巨型生物到达，请注意防护")
            inst:DoTaskInTime(start_time_delay,function()
                if not inst.components.hoshino_data:Get("boss_spawned") then
                        ...
                        if type(pt) == type(Vector3(0,0,0)) then
                            local boss = SpawnPrefab("toadstool_dark")
                            boss.Transform:SetPosition(pt.x,0,pt.z)
                            local test_num = 100
                            local debuff_prefab = "hoshino_mission_golden_13_debuff"
                            while test_num > 0 do
            ...
            --- 击杀事件
            inst:ListenForEvent("hoshino_mission_golden_13_kill_with_debuff",function()
...
return Prefab("hoshino_mission_golden_13", fn, assets),
    Prefab("hoshino_mission_golden_13_debuff", debuff_fn, assets)