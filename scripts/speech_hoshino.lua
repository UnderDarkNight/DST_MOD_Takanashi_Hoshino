...
return
{
	ACTIONFAIL =
	{
		...
		REPAIR =	--修理
		{
			WRONGPIECE = "呜诶，好像错了？。",		--雕像或化石骨架拼接错误
		},
		BUILD =
		{
			MOUNTED = "得下去才行呢。",		--骑乘状态下无法建造
			HASPET = "不能抛弃自己的宠物呢。",		--建造失败，已经有一个宠物了
			TICOON = "已经有一只啦。",	--建造失败，已经有一只大虎了
		},
		SHAVE =
		{
			AWAKEBEEFALO = "啊...会有危险吧。",		--给醒着的牛刮毛
			GENERIC = "老师可能会这个吧~",		--刮牛毛失败
			NOBITS = "已经一点都没有啦！",		--不能给没毛的牛刮毛
			REFUSE = "only_used_by_woodie",		--拒绝刮胡子（伍迪专用）
			SOMEONEELSESBEEFALO = "这是朋友的噢！",		--不能刮队友牛的毛
		},
		STORE =
		{
			GENERIC = "已经满了噢。",		--存放东西失败
			NOTALLOWED = "放错了呢。",		--存放东西不被允许
			INUSE = "嘛，再等等吧。",		--别人正在用箱子
			NOTMASTERCHEF = "大叔我还不会使用这个呢~。",		--使用大厨厨具
		},
		CONSTRUCT =		--建造计划
		{
			INUSE = "等等别人吧~。",		--建筑正在使用
			NOTALLOWED = "好像错了呢",		--材料不对
			EMPTY = "好像没有原材料呢~。",		--材料栏无材料
			MISMATCH = "哎呀，计划错误。",		--建造成果错误（未应用）
			NOTREADY = "现在不是干这个的时候。",	--当前状态无法建造
		},
		RUMMAGE =	--打开或关闭容器
		{
			GENERIC = "我不能那么做。",		--打开箱子失败
			INUSE = "别人还在用呢，再等等吧~",		--不能打开箱子，别人正在使用
			NOTMASTERCHEF = "大叔我还不会使用这个呢~",		--使用大厨厨具
		},
		...
		{
			WRONGKEY = "好像错了？",		--使用普通鹿角
			KLAUS = "得先解决这个大家伙呢",		--克劳斯在场，无法打开
		...
		{
			PROFESSIONALCHEF = "大叔我还不会使用这个呢~",		--非大厨角色使用便携厨具
			SHADOWMAGIC = "这种魔法，没办法理解",	--非老麦角色使用暗影秘典
		},
		COOK =
		{
			GENERIC = "我现在做不了饭。",		--做饭失败
			INUSE = "别人还在用呢，再等等吧~",		--做饭失败，别人在用锅
		...
		GIVE =
		{
			...
			WRONGSHADOWFORM = "拼错了呢。",		--化石骨架形态错误，无法用心脏复活
			...
			NOTAMONKEY = "没法交流呢。",	--非芜猴玩家不能给予月亮码头女王物品
			QUEENBUSY = "她好像在忙。",	--月亮码头女王忙碌中无法接受物品
		},
		GIVETOPLAYER =
		{
			FULL = "他拿不下了。",		--给玩家一个东西，但是背包满了
			DEAD = "这个不是复活道具呢。",		--给死亡的玩家一个东西
			SLEEPING = "大叔也想睡觉呢~",		--给睡觉的玩家一个东西
			BUSY = "得等他腾出手呢",		--给忙碌的玩家一个东西
		},
		GIVEALLTOPLAYER =
		{
			FULL = "他拿不下了。",		--给人一组东西，但是背包满了
			DEAD = "这个不是复活道具呢。",		--给于死去的玩家一组物品
			SLEEPING = "大叔也想睡觉呢~",		--给于正在睡觉的玩家一组物品
			BUSY = "得等他腾出手呢",		--给于正在忙碌的玩家一组物品
		},
		WRITE =
		{
		...
		ATTUNE =
		{
			NOHEALTH = "啊，得恢复恢复",		--制造肉雕像血量不足
		},
		MOUNT =
		{
			TARGETINCOMBAT = "我应该去帮帮它。",		--骑乘失败，牛正在战斗
			INUSE = "别人先上去了！",		--骑乘失败，牛被占据
			SLEEPING = "我也想睡会呢~",	--牛在睡觉
		},
		SADDLE =
		{
		...
		ROW_FAIL =		--划船失败
		{
			BAD_TIMING0 = "平时没练过这个呢。",		--失败语句一
			BAD_TIMING1 = "哎呀！",		--失败语句二
			BAD_TIMING2 = "衣服湿掉了！",		--失败语句三
		},
	...
	{
		NO_INGREDIENTS = "材料不够啊~",	--材料不足
		NO_TECH = "这个可能需要交给千年的各位呢~",	--科技不够
		NO_STATION = "这个需要制作站哦~",	--不在科技建筑旁边
	},

	ACTIONFAIL_GENERIC = "做不到呢~",		--动作失败
	ANNOUNCE_BOAT_LEAK = "水漫进来了！",		--船上出现了新的漏洞
	ANNOUNCE_BOAT_SINK = "快要沉下去了啊！",		--船正在裂开，马上要沉了
	...
	ANNOUNCE_EAT =
	{
		...
		INVALID = "坚决不会吃这个。",		--不可食用
		YUCKY = "快拿开啊啊啊啊！",		--拒绝食用恶心的东西（比如永恒的水果蛋糕）
	...
	ANNOUNCE_DAMP = "雨好大~",		--潮湿（1级）
	ANNOUNCE_WET = "衣服湿透了呢，老师可不许偷看哦~",		--潮湿（2级）
	ANNOUNCE_WETTER = "这样下去会感冒吧！",		--潮湿（3级）
	ANNOUNCE_SOAKED = "大叔我得找个地方躲躲了",		--潮湿（4级）

	ANNOUNCE_WASHED_ASHORE = "诶，我飘回来了？",		--淹水后飘回岸上

	ANNOUNCE_DESPAWN = "呜啊啊啊啊",		--天体传送门处换人
	ANNOUNCE_BECOMEGHOST = "~~~~~~~~",		--变成鬼魂
	ANNOUNCE_GHOSTDRAIN = "我会救你的，一定会。",		--队友死亡变成鬼魂掉san
	ANNOUNCE_PETRIFED_TREES = "诶？树木变成了石头诶！",		--树木石化了
	ANNOUNCE_KLAUS_ENRAGE = "就这？大叔我有点失望呢~",		--杀死了鹿后克劳斯被激怒
	ANNOUNCE_KLAUS_UNCHAINED = "再给你一次机会，也没用哦。",		--克劳斯第一次死亡后复活
	ANNOUNCE_KLAUS_CALLFORHELP = "这是在求救吗？",		--克劳斯召唤小偷
	...
	ANNOUNCE_ATTACH_BUFF_ELECTRICATTACK	= "诶？我也可以使用闪电了呢！",		--获得料理buff：带电攻击
	ANNOUNCE_ATTACH_BUFF_ATTACK			= "前卫就交给我了~",		--获得料理buff：增加伤害
	ANNOUNCE_ATTACH_BUFF_PLAYERABSORPTION  = "哦呀？好像盾牌更牢固了",		--获得料理buff：增加防御
	ANNOUNCE_ATTACH_BUFF_WORKEFFECTIVENESS = "工作时间到~",		--获得料理buff：工作效率提高
	ANNOUNCE_ATTACH_BUFF_MOISTUREIMMUNITY  = "这下可以防水啦。",		--获得料理buff：防潮湿
	ANNOUNCE_ATTACH_BUFF_SLEEPRESISTANCE   = "还是想睡觉呢~",		--获得料理buff：睡眠抗性

	ANNOUNCE_DETACH_BUFF_ELECTRICATTACK	= "闪电的力量消失了~",		--料理buff消失：带电攻击
	ANNOUNCE_DETACH_BUFF_ATTACK			= "唔...没那么有力了呢",		--料理buff消失：增加伤害
	ANNOUNCE_DETACH_BUFF_PLAYERABSORPTION  = "虽然时间不长，但是还不错。",		--料理buff消失：增加防御
	ANNOUNCE_DETACH_BUFF_WORKEFFECTIVENESS = "可以休息啦。",		--料理buff消失：工作效率提高
	ANNOUNCE_DETACH_BUFF_MOISTUREIMMUNITY  = "又要被淋湿啦。",		--料理buff消失：防潮湿
	ANNOUNCE_DETACH_BUFF_SLEEPRESISTANCE   = "可以睡觉了呢~",		--料理buff消失：睡眠抗性

	ANNOUNCE_OCEANFISHING_LINESNAP = "啊，这下看不到鱼了。",		--海钓竿断线
	ANNOUNCE_OCEANFISHING_LINETOOLOOSE = "现在我应该把它拉起来",		--咬钩的海鱼不扯线
	...
	ANNOUNCE_WINCH_CLAW_NO_ITEM = "又没找到宝藏呢~",		--夹夹绞盘没捞到东西
	...
	BATTLECRY =
	{
		GENERIC = "准备作战。",		--战斗
		PIG = "此路不通！",		--与猪人战斗（疯猪除外）
		PREY = "那么，一口气结束战斗吧~",		--与小型生物战斗（例如兔子，青蛙，鼹鼠）
		SPIDER = "唔诶，在这里也要战斗啊",		--与蜘蛛战斗
		SPIDER_WARRIOR = "这么大的蜘蛛我也是会怕的啊~",		--与蜘蛛战士战斗
		DEER = "借你的角用用哦。",		--与无眼鹿战斗
	},
	COMBAT_QUIT =
	{
		...
		PIG = "先去休息下吧。",		--停止攻击猪人
		PREY = "收工吧~",		--停止攻击小型生物
		SPIDER = "先不管他们了",		--停止攻击蜘蛛
		SPIDER_WARRIOR = "撤退咯。",		--停止攻击蜘蛛战士
	},
	...
	DESCRIBE =
	{
		...
		STATUEGLOMMER =
		{
			GENERIC = "这个是什么呢~",		-- 物品名:"格罗姆雕像"->默认
			EMPTY = "啊，坏掉了呢。",		-- 物品名:"格罗姆雕像"->被开采
		},
		...
		WORM =
		{
			...
			WORM = "好大的虫子啊！",		-- 物品名:"洞穴蠕虫"->钻出来了
		},
		WORMLIGHT_PLANT = "我觉得挺安全。",		-- 物品名:"神秘植物"（光莓）
		MOLE =
		{
			HELD = "很少见呢。",		-- 物品名:"鼹鼠"->拿在手里
		...
		MUSHROOM_LIGHT =
		{
			ON = "好，亮起来了。",		-- 物品名:"蘑菇灯"->开启 制造描述:"任何蘑菇的完美添加物。"
		...
		TOADSTOOL =
		{
			GENERIC = "好大的蟾蜍啊！",		-- 物品名:"毒菌蟾蜍"->默认
			RAGE = "它生气了！",		-- 物品名:"毒菌蟾蜍"->愤怒
		},
		MUSHROOMSPROUT =
		{
		...
		BABYBEEFALO =
		{
			...
			SLEEPING = "看着我也想去睡觉呢~",		-- 物品名:"小皮弗娄牛"->睡着了
		},
		...
		BEEQUEENHIVEGROWN = "这里面会有多少蜂蜜啊！",		-- 物品名:"巨大蜂窝"
		BEEGUARD = "你也在保护自己重要的人吗？",		-- 物品名:"嗡嗡蜜蜂"
		HIVEHAT = "当我穿上它时，这世界看起来不那么疯狂了。",		-- 物品名:"蜂王冠"
		MINISIGN =
		{
			GENERIC = "这样就知道这是什么了",		-- 物品名:"小木牌"->画上了东西
		...
		MUSHROOM_FARM =
		{
			...
			BURNT = "生长吧。",		-- 物品名:"蘑菇农场"->烧焦的 制造描述:"种蘑菇。"
		...
		BIRDCAGE =
		{
			...
			OCCUPIED = "现在它也有家了。",		-- 物品名:"鸟笼"->有鸟 制造描述:"为你的鸟类朋友提供一个幸福的家。"
			...
			DEAD = "我会送你离开。",		-- 物品名:"鸟笼"->死了 制造描述:"为你的鸟类朋友提供一个幸福的家。"
			SKELETON = "走好。",		-- 物品名:"鸟笼"->骷髅
		},
		...
		CANARY =
		{
			...
			HELD = "这个可以放到鸟笼里吗？",		-- 物品名:"金丝雀"->拿在手里
		},
		CANARY_POISONED = "它可能时日无多了。",		-- 物品名:"金丝雀（中毒）"
		...
		LAVAE_PET =
		{
			...
			GENERIC = "为什么它叫超级可爱岩浆虫呢？",		-- 物品名:"超级可爱岩浆虫"->默认
		},
		LAVAE_EGG =
		{
