...
----- 用于新大型版本管理和上线后文件切割。修BUG不用担心新的未完成内容上线造成崩溃。
	TUNING.HOSHINO_DEBUGGING_MODE = TUNING.HOSHINO_DEBUGGING_MODE or GetModConfigData("DEBUGGING_MOD") or false	
		if TUNING.HOSHINO_DEBUGGING_MODE then
			AddPlayerPostInit(function(player_inst)	---- 玩家进入后再执行。检查。
				...
				player_inst:DoTaskInTime(2,function()
					TheNet:SystemMessage("小鸟游星野 测试模式 ON")				
				end)
			end)
		end
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
modimport("imports_for_hoshino/__all_imports_init.lua")	---- 所有 import  文本库（语言库），素材库
...
--- 获取mod 版本
	...
	TUNING["hoshino.mod_info"] = mod_info
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------


modimport("key_modules_for_hoshino/_all_key_modules_init.lua")	---- 载入关键功能模块,在 prefab 加载之前，方便皮肤的API HOOK

PrefabFiles = {  "hoshino__all_prefabs"  }		---- 通过总入口 加载所有prefab。
