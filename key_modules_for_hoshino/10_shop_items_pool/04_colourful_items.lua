TUNING.HOSHINO_SHOP_ITEMS_POOL = TUNING.HOSHINO_SHOP_ITEMS_POOL or {}
TUNING.HOSHINO_SHOP_ITEMS_POOL["colourful"] = {
-----------------------------------------------------------
--  security_pulse_cage
{
  prefab = "security_pulse_cage",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("security_pulse_cage.tex"), image = "security_pulse_cage.tex"},
  price = 1250, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "normal", -- 类型。normal special 。这个可以不下发。
  is_permanent = true, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  opalpreciousgem
{
  prefab = "opalpreciousgem",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("opalpreciousgem.tex"), image = "opalpreciousgem.tex"},
  price = 1250, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "normal", -- 类型。normal special 。这个可以不下发。
  is_permanent = true, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  moonglass_charged
{
  prefab = "moonglass_charged",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("moonglass_charged.tex"), image = "moonglass_charged.tex"},
  price = 90, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "normal", -- 类型。normal special 。这个可以不下发。
  is_permanent = true, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  thulecite
{
  prefab = "thulecite",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("thulecite.tex"), image = "thulecite.tex"},
  price = 320, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "normal", -- 类型。normal special 。这个可以不下发。
  is_permanent = true, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  voidcloth
{
  prefab = "voidcloth",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("voidcloth.tex"), image = "voidcloth.tex"},
  price = 660, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "normal", -- 类型。normal special 。这个可以不下发。
  is_permanent = true, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  lunarplant_husk
{
  prefab = "lunarplant_husk",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("lunarplant_husk.tex"), image = "lunarplant_husk.tex"},
  price = 450, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "normal", -- 类型。normal special 。这个可以不下发。
  is_permanent = true, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  purebrilliance
{
  prefab = "purebrilliance",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("purebrilliance.tex"), image = "purebrilliance.tex"},
  price = 660, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "normal", -- 类型。normal special 。这个可以不下发。
  is_permanent = true, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  horrorfuel
{
  prefab = "horrorfuel",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("horrorfuel.tex"), image = "horrorfuel.tex"},
  price = 450, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "normal", -- 类型。normal special 。这个可以不下发。
  is_permanent = true, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  hoshino_item_abydos_high_purity_alloy
{
  prefab = "hoshino_item_abydos_high_purity_alloy",
  bg = "item_slot_colourful.tex",
  icon = {atlas = "images/inventoryimages/hoshino_item_abydos_high_purity_alloy.xml", image = "hoshino_item_abydos_high_purity_alloy.tex"},
  price = 4000, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "normal", -- 类型。normal special 。这个可以不下发。
  is_permanent = true, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  opalstaff
{
  prefab = "opalstaff",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("opalstaff.tex"), image = "opalstaff.tex"},
  price = 2000, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "special", -- 类型。normal special 。这个可以不下发。
  is_permanent = false, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  shadowheart_infused
{
  prefab = "shadowheart_infused",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("shadowheart_infused.tex"), image = "shadowheart_infused.tex"},
  price = 1, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "laplite", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "special", -- 类型。normal special 。这个可以不下发。
  is_permanent = false, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  alterguardianhat
{
  prefab = "alterguardianhat",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("alterguardianhat.tex"), image = "alterguardianhat.tex"},
  price = 4500, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "special", -- 类型。normal special 。这个可以不下发。
  is_permanent = false, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  armorskeleton
{
  prefab = "armorskeleton",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("armorskeleton.tex"), image = "armorskeleton.tex"},
  price = 3000, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "special", -- 类型。normal special 。这个可以不下发。
  is_permanent = false, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  thurible
{
  prefab = "thurible",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("thurible.tex"), image = "thurible.tex"},
  price = 3000, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "special", -- 类型。normal special 。这个可以不下发。
  is_permanent = false, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  hoshino_item_accessory_remnants
{
  prefab = "hoshino_item_accessory_remnants",
  bg = "item_slot_colourful.tex",
  icon = {atlas = "images/inventoryimages/hoshino_item_accessory_remnants.xml", image = "hoshino_item_accessory_remnants.tex"},
  price = 9, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "laplite", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "special", -- 类型。normal special 。这个可以不下发。
  is_permanent = false, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  alterguardianhatshard
{
  prefab = "alterguardianhatshard",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("alterguardianhatshard.tex"), image = "alterguardianhatshard.tex"},
  price = 1666, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "special", -- 类型。normal special 。这个可以不下发。
  is_permanent = false, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  armorskeleton
{
  prefab = "armorskeleton",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("armorskeleton.tex"), image = "armorskeleton.tex"},
  price = 1, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "laplite", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "special", -- 类型。normal special 。这个可以不下发。
  is_permanent = false, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  thurible
{
  prefab = "thurible",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("thurible.tex"), image = "thurible.tex"},
  price = 1, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "laplite", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "special", -- 类型。normal special 。这个可以不下发。
  is_permanent = false, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  alterguardianhatshard
{
  prefab = "alterguardianhatshard",
  bg = "item_slot_colourful.tex",
  icon = {atlas = GetInventoryItemAtlas("alterguardianhatshard.tex"), image = "alterguardianhatshard.tex"},
  price = 1, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "laplite", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "special", -- 类型。normal special 。这个可以不下发。
  is_permanent = false, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  hoshino_item_cards_pack_supreme_mystery
{
  prefab = "hoshino_item_cards_pack_supreme_mystery",
  bg = "item_slot_colourful.tex",
  icon = {atlas = "images/inventoryimages/hoshino_item_cards_pack_supreme_mystery.xml", image = "hoshino_item_cards_pack_supreme_mystery.tex"},
  price = 2, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "laplite", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "special", -- 类型。normal special 。这个可以不下发。
  is_permanent = false, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  hoshino_item_nine
{
  prefab = "hoshino_item_nine",
  bg = "item_slot_colourful.tex",
  icon = {atlas = "images/inventoryimages/hoshino_item_nine.xml", image = "hoshino_item_nine.tex"},
  price = 3000, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "special", -- 类型。normal special 。这个可以不下发。
  is_permanent = false, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  hoshino_item_sky
{
  prefab = "hoshino_item_sky",
  bg = "item_slot_colourful.tex",
  icon = {atlas = "images/inventoryimages/hoshino_item_sky.xml", image = "hoshino_item_sky.tex"},
  price = 3000, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "special", -- 类型。normal special 。这个可以不下发。
  is_permanent = false, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  hoshino_item_spring
{
  prefab = "hoshino_item_spring",
  bg = "item_slot_colourful.tex",
  icon = {atlas = "images/inventoryimages/hoshino_item_spring.xml", image = "hoshino_item_spring.tex"},
  price = 3000, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "special", -- 类型。normal special 。这个可以不下发。
  is_permanent = false, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
--  hoshino_item_snow
{
  prefab = "hoshino_item_snow",
  bg = "item_slot_colourful.tex",
  icon = {atlas = "images/inventoryimages/hoshino_item_snow.xml", image = "hoshino_item_snow.tex"},
  price = 3000, -- 价格
  num_to_give = 1, -- 单次购买的数量。【注意】nil 自动处理为1。
  price_type = "credit_coins", -- 货币需求。
  level = 3, -- 等级限制。这个可以不下发。用来解锁。 【注意】做nil 自动处理为0。
  type = "special", -- 类型。normal special 。这个可以不下发。
  is_permanent = false, -- 是否永久。0 非永久 1 永久
},
-----------------------------------------------------------
}
