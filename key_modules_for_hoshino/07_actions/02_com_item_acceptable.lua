...
local HOSHINO_COM_ACCEPTABLE_ACTION = Action({priority = 10})   --- 距离 和 目标物体的 碰撞体积有关，为 0 也没法靠近。
HOSHINO_COM_ACCEPTABLE_ACTION.id = "HOSHINO_COM_ACCEPTABLE_ACTION"
HOSHINO_COM_ACCEPTABLE_ACTION.strfn = function(act) --- 客户端检查是否通过,同时返回显示字段
    local item = act.invobject
    local target = act.target
    local doer = act.doer
    if doer and target and target.replica.hoshino_com_acceptable then
        local replica_com = target.replica.hoshino_com_acceptable or target.replica._.hoshino_com_acceptable
        if replica_com then
...
HOSHINO_COM_ACCEPTABLE_ACTION.fn = function(act)    --- 只在服务端执行~
    local item = act.invobject
    local target = act.target
    local doer = act.doer
    if item and target and doer and target.components.hoshino_com_acceptable then
        local replica_com = target.replica.hoshino_com_acceptable or target.replica._.hoshino_com_acceptable
        if replica_com and replica_com:Test(item,doer,true) then
            return target.components.hoshino_com_acceptable:OnAccept(item,doer)
        end
    end
    return false
end
AddAction(HOSHINO_COM_ACCEPTABLE_ACTION)
...
AddComponentAction("USEITEM", "inventoryitem", function(item, doer, target, actions, right_click) -- -- 一个物品对另外一个目标用的技能，
    if doer and item and target then
        local hoshino_com_acceptable_com = target.replica.hoshino_com_acceptable or target.replica._.hoshino_com_acceptable
        if hoshino_com_acceptable_com and hoshino_com_acceptable_com:Test(item,doer,right_click) then
            table.insert(actions, ACTIONS.HOSHINO_COM_ACCEPTABLE_ACTION)
        end        
    end
end)
...
local function handler_fn(player)
    local creash_flag , ret = pcall(function()
        ...
        local replica_com = target and ( target.replica.hoshino_com_acceptable or target.replica._.hoshino_com_acceptable )
        if replica_com then
    ...
    if creash_flag == true then
        ...
        print("error in HOSHINO_COM_ACCEPTABLE_ACTION ActionHandler")
        print(ret)
    end
    return "give"
end

AddStategraphActionHandler("wilson",ActionHandler(HOSHINO_COM_ACCEPTABLE_ACTION,function(player)
    return handler_fn(player)
end))
AddStategraphActionHandler("wilson_client",ActionHandler(HOSHINO_COM_ACCEPTABLE_ACTION, function(player)    
    return handler_fn(player)
end))
