...
--------------------------------------------------------------------------------------
    TheInput.__hoshino_update_custom_fns = {}
    TheInput.__hoshino_remove_fn = function(inst)
        TheInput:Hoshino_Remove_Update_Custom_Fn(inst)
    end
    function TheInput:Hoshino_Add_Update_Custom_Fn(inst,fn)
        if inst and type(fn) == "function" then
            self.__hoshino_update_custom_fns[inst] = fn
        end
        inst:ListenForEvent("onremove",self.__hoshino_remove_fn)
        -- print("TheInput add update modify fn",inst)
    end
    function TheInput:Hoshino_Remove_Update_Custom_Fn(inst)
        local new_table = {}
        for k,v in pairs(self.__hoshino_update_custom_fns) do
            if k ~= inst then
                new_table[k] = v
            end
        end
        self.__hoshino_update_custom_fns = new_table
